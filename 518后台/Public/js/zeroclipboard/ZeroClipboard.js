...
var __FILE__ =js[js.length-1].src.substring(0,js[js.length-1].src.lastIndexOf("/")+1);

var clipboard_class = function(dom) {
	this.dom = dom;
} 

clipboard_class.prototype.cut = function(val) {
	
	
	this.clip = new ZeroClipboard(this.dom, {
		moviePath: __FILE__ + "ZeroClipboard.swf"
	} );
	
	if (typeof this.fn == 'function') {
		var fn = this.fn;
	} else {
		var fn = function(client, args) {
			alert("复制成功");
		}
	}
	
	this.clip.on('complete', fn);
	this.dom.setAttribute("data-clipboard-text", val);
}

clipboard_class.prototype.on = function(event, fn) {
	if (event == 'complete') {
		this.fn = fn;
	} else {
		return;
...
var ZC = function (id) {
	...
	if (typeof zc_obj[id] != 'object') {
		zc_obj[id] = new clipboard_class(dom); 
	}
	return zc_obj[id]; 
}
