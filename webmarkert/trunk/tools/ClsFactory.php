...
function do_login_request($data, $update_user_info = false){
	...
	$discuz_api = "http://bbs.anzhi.com/api/user.php?action=login";
	$error = 0;
	...
	if ($error){
		...
		$postdata = array('username' => base64_encode(mb_convert_encoding($username, 'GBK', 'utf-8')), 'password' => base64_encode($password),);
...
function getCacheData($item, $option = ""){
	...
	if (!array_key_exists($var_key, $GLOBALS)){
		...
		$softObj = ClsFactory :: Create('model::nSoft');
...
function check_if_user_is_dev(){
	...
	if (intval($info['status']) ==-1){
		exitOnError("您的账户已经被拒绝访问,请联系安智客服人员", '/login.php');
	}
...
if (!function_exists('mygetcsv')):
     ...
     function http_post($vals = array(), $files = array()){
		if($_SERVER['SERVER_ADDR']=='192.168.0.99') {
		...
		} else if($GLOBALS["GO_ENV"] == GO_ENV_PRODUCT) {
     		$host = '192.168.1.18';
			$host_dam = 'Host: dummy.goapk.com';
		} else {
			$host = '192.168.0.99';
         ...
         file_put_contents('/tmp/httppost.log', var_export(curl_getinfo($result), true), FILE_APPEND);		 
         if (!$result){
     ...
     #go_move("apk", "${md5}.apk", STATIC_DATA,"${package}_${md5}.apk")
     ...
     function go_apkinfo($file){
		...
		file_put_contents("/tmp/lipeng.log",  var_export($res,true). "\n", FILE_APPEND);
     	if (!$res){
     		return false;
     	}
     	$vals = array(
 ...
 */
function __mkdir($path, $mod = '0755', $recursive = false)
{
	static $loop = 0;
	$loop ++;
	...
	return __mkdir(dirname($path)) && __mkdir($path, $mod, $recursive);
}
endif;
