<?php
class ClientlistAction extends CommonAction{
	...
	function add_client_do(){
		...
		if($result){
			...
			if($go == 1){
				$this -> assign('jumpUrl','/index.php/Channel_cooperation/Clientlist/client_list');
			}else{
				$this -> assign('jumpUrl','/index.php/Channel_cooperation/Clientlist/add_channel_show/id/'.$result);
			}
			$this -> success("添加成功");
		}else{
			$this -> error("添加失败");
	...
	function edit_client_do(){
		...
		if($result){
			...
			if($go == 1){
				...
				$this -> assign('jumpUrl','/index.php/Channel_cooperation/Clientlist/add_channel_show/id/'.$id);
			}
			$this -> success("编辑成功");
		}else{
			$this -> error("编辑失败");
	...
	function add_channel_show(){
		...
		$where['_string'] = "status = 1 and co_status = 3 ".$where_go;
		//屏蔽用户开关新增
	...
	function channel_list(){
		...
		if($my_status){
			$where_go .= " and co_status = {$my_status}";
		}
		if($chname){
	...
	function change_channel(){
		...
		$where['cid'] = $cid;
		if($status == 2){
			$coefficient_result = $model -> table('sj_channel_coefficient') -> where(array('cid' => $cid,'status' => 1)) -> save(array('status' => 0,'last_refresh' => time()));
			...
			$coefficient_change_result = $model -> table('sj_channel_coefficient') -> add($coefficient_data);
			$admin_id = $_SESSION['admin']['admin_id'];
			$admin_ip = $this -> getIP();
			$log_data = array(
				'cid' => $cid,
		...
		if($result){
			...
			$this -> assign('jumpUrl','/index.php/Channel_cooperation/Clientlist/channel_list'.$where_go);
			$this -> success("编辑成功");
		}else{
			$this -> error("编辑失败");
		}
