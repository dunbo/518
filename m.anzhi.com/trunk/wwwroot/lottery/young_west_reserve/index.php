...
$login_url = $center_url."http://promotion.anzhi.com/lottery/young_west_reserve/index.php?lgfrom=1&".$build_query;
...
if((isset($_SESSION['USER_UID']) && $_SESSION['USER_ID'] != 13176))//已登录
{
	...
	//todo 是否预约
	$res = $redis->get("young_west_reserve:{$active_id}_is_sign:".$uid);
	if($res == 1)
	{
	...
	else
	{
		if($_GET['lgfrom']) //不管是登录还是注册都是预约成功的
		{
			...
			$redis->set("young_west_reserve:{$active_id}_is_sign:".$uid,1,86400*30);
...
$tpl = "lottery/young_west_reserve/index.html";
if($_SERVER['SERVER_ADDR'] == '118.26.203.23')
{
...
$tplObj -> out['share_text'] = "西游卡牌王者之作，预约领取独家豪华大礼包！";
