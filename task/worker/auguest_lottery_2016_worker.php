...
$worker->addFunction("auguest_lottery_2016", "get_award");
while ($worker->work());
...
function get_award($jobs){
	...
	if($money >= 6 and $money < 599){
		$probability_type = 1;
	}else if($money >= 599 and $money < 999){
		$probability_type = 2;
	}else if($money >= 999 and $money < 2999){
		$probability_type = 3;
	}else if($money >= 2999 and $money < 4999){
		$probability_type = 4;
	}else if($money >= 4999 and $money < 9999){
		$probability_type = 5;
	}else{
		$probability_type = 6;
	}
	...
	if(empty($basenum) || empty($gift_base)){
		...
		foreach($prize_arr as $v){
			...
			//处理中奖率
			//list($probability,$probability1,$probability2,$probability3,$probability4,$probability5) = explode(';',$v['probability']);	
			$pro_arr  = explode(';',$v['probability']);	
                        if($pro_arr[$probability_type-1]==0) continue;
			$rs = explode('/',$pro_arr[$probability_type-1]);	
	...
	if($pid!=-1){
		...
		if($type != 2){
			...
			$task_client->doBackground('auguest_2016_db',json_encode($param));
			$resarr = array(
				'pid' => $pid,
				'prize_rank' => $prize_rank,
				'prizename' => $prizename,
