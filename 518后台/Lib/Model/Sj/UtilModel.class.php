...
class UtilModel extends Model {
	...
	//V6.4 获取合作站点
	public function getCoopSiteList()
	{
		//内容合作  展示频道、标签和合作站点
		$coop_where=array(
			'status'=>1,
			'type'=>array('not in','3,4,5')
		);
		$coop_site = $this->table('coop_site')->where($coop_where)->select();
		return $coop_site;
	}
	//根据站点id获取站点名称
	public function getCoopSiteName($id)
	{
		//内容合作  展示频道、标签和合作站点
		$coop_where=array(
			'status'=>1,
		...
		$coop_site = $this->table('coop_site')->where($coop_where)->find();
		return $coop_site['website_name'];
	}
	public function getHomeExtentSoftTypeList($type = 0)
	{
		$typelist = array(
			...
			15 => array('礼包',$type == 15),
			16 => array('新服',$type == 16),
			17 => array('CPD广告',$type == 17),
			18 => array('屏蔽',$type == 18),
			19 => array('CPS商务广告',$type == 19),
		);
		return $typelist;
	}
	//批量上传用到合作形式
	public function get_cooperation()
	{
		$typelist= $this->getHomeExtentSoftTypeList();
		$new_coop = array();
	...
	public function getOperators($user_selected = array())
	{
		...
		$result[-1] = array('未插卡', isset($selected[-1]));
	...
	public function getMarketVersion($user_selected = array())
	{
		...
		$market_list = $this->table('sj_market')->field('real_version_code,version_name')->where('status=1 and real_version_code>=3900')->order('real_version_code asc')->group('real_version_code')->select();
		$version_list = array();
		/*
		$version_list = array(
		...
		*/
		...
		foreach ($market_list as $val) {
			$version_list[$val['real_version_code']] = array("{$val['version_name']} ({$val['real_version_code']})", isset($selected[$val['real_version_code']]));
		}
		return $version_list;
	}
