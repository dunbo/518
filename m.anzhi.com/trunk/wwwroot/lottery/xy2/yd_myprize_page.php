...
if($_POST){
		$kind_award_gift = get_user_kind_gift_new_yd(1,$uid,$active_id,"yd","xy2_draw_gift");	

		$prize_list = get_user_kind_award_new_yd(1,$uid,$active_id,"yd","xy2_draw_award");	

                $kind_award_gift_ka = get_user_kind_gift_new_yd(2,$uid,$active_id,"yd","xy2_draw_gift");	

		$prize_list_ka = get_user_kind_award_new_yd(2,$uid,$active_id,"yd","xy2_draw_award");	
                if(empty($kind_award_gift)&&empty($prize_list)&&empty($kind_award_gift_ka)&&empty($prize_list_ka)){
                    echo 1;exit(0);
                }else{
                    echo 2;exit(0);
                }
	...
	$tplObj -> out['nowday'] = $nowday;
		...
		$prize_list = get_user_kind_award_new_yd(1,$uid,$active_id,"yd","xy2_draw_award");	
                ...
                $kind_award_gift_ka = get_user_kind_gift_new_yd(2,$uid,$active_id,"yd","xy2_draw_gift");	
		$tplObj -> out['gift_prize_arr_ka'] = $kind_award_gift_ka;

		$prize_list_ka = get_user_kind_award_new_yd(2,$uid,$active_id,"yd","xy2_draw_award");	
		$tplObj -> out['prize_list_ka'] = $prize_list_ka;
	...
	$tplObj -> display('lottery/xy2/yd_myprize_page.html');
