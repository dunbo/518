<?php
	...
	if($res_arr['result'] != 1){
		...
		$redis->expire($send_code_tm_key,86400*2);
		if(!$is_callback){
			//判断当前订单是否已经回滚过了如已经回滚不执行下面的操作
			return array('code'=>0,'msg'=>'提取失败');	
		}

                //$day_use_num_key = $prefix.":".$active_id.":day_use_num:".$imsi.":day:".date('Ymd');
	...
	$model->update($where, $data, 'lottery/lottery');	
        $is_get_ll_pkg_key = $prefix.":".$active_id.":is_get_ll_pkg:".$package.":imsi:".$imsi;
        $redis->setnx($is_get_ll_pkg_key,1);
        $redis->expire($is_get_ll_pkg_key,86400*30);
