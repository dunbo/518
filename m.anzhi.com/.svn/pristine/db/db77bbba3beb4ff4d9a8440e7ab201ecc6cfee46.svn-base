...
$rkey_imsi_ignite_love = "christmas2015:{$aid}:{$imsi}:ignite_love";//用户最新一次用爱点亮圣诞树的日期
$rkey_imsi_ignite_share = "christmas2015:{$aid}:{$imsi}:ignite_share";//用户最新一次分享的日期
$rkey_imsi_lottery_num = "christmas2015:{$aid}:{$imsi}:lottery_num";//用户可抽奖次数
$rkey_imsi_package_list = "christmas2015:{$aid}:{$imsi}:package_list";
...
function get_lottery_num() {
	...
	if (empty($lottery_num) && $lottery_num !== 0) {
		...
		$option = array(
			...
			'table' => 'christmas2015_lottery_num',
		);
		...
		if ($find) {
			...
			$data = array(
				...
				'time' => time(),
				'__user_table' => 'christmas2015_lottery_num',
			);
