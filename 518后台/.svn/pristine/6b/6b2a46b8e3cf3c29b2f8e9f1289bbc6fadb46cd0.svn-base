...
class  MgnewsAction extends CommonAction{

	function mgnews_list(){
		...
		$count = $bbs_model -> table('bbs_mgnews_pic') -> where($where) -> count();
		import("@.ORG.Page");
		$param = http_build_query($_GET);
		$Page = new Page($count, 20, $param);
		$result = $bbs_model -> table('bbs_mgnews_pic') -> where($where) -> limit($Page->firstRow . ',' . $Page->listRows) -> order('rank,create_tm DESC') -> select();
		if($_GET['lr']){
			$lr = $_GET['lr'];
		}else{
			$lr = 20;
	...
	function add_mgnews_show(){
		$this -> display();
	}
	
	function add_mgnews_do(){
		$bbs_model = D('Bbs_manage.Bbs_manage');
		$pic_title = $_POST['pic_title'];
		if(mb_strlen($pic_title,'utf8') > 20){
		...
		if ($my_pic['size']) {
            ...
            if ($width_go_ha != 317 || $height_go_ha != 136) {
		...
		$data['update_tm'] = time();
		$data['status'] = 1;
		$result = $bbs_model -> table('bbs_mgnews_pic') -> add($data);
		if($result){
			$this -> writelog("已添加id为{$result}的手游资讯图","bbs_mgnews_pic",$result,__ACTION__ ,"","add");
			$this -> assign("jumpUrl","/index.php/Bbs_manage/Mgnews/mgnews_list");
			$this -> success("添加成功");
		}else{
			$this -> error("添加失败");
		}
	...
	function edit_mgnews_show(){
		...
		$lr = $_GET['lr'];
		$p = $_GET['p'];
		$result = $bbs_model -> table('bbs_mgnews_pic') -> where(array('id' => $id)) -> find();
		$this -> assign("result",$result);
		$this -> assign("pic_title",$pic_title);
		$this -> assign("start_tm",$start_tm);
		$this -> assign("end_tm",$end_tm);
	...
	function edit_mgnews_do(){
		...
		$data['pic_link'] = $pic_link;
		$data['update_tm'] = time();
		$log_result = $this -> logcheck(array('id' => $id),'bbs_mgnews_pic',$data,$bbs_model);
		$result = $bbs_model -> table('bbs_mgnews_pic') -> where(array('id' => $id)) -> save($data);
		if($result){
			$this -> writelog("已编辑id为{$id}的手游资讯图".$log_result,"bbs_mgnews_pic",$id,__ACTION__ ,"","edit");
			$this -> assign("jumpUrl","/index.php/Bbs_manage/Mgnews/mgnews_list".$where_go);
			$this -> success("编辑成功");
		}else{
			$this -> error("编辑失败");
		}
	...
	function del_mgnews_pic(){
		...
		$result = $bbs_model -> table('bbs_mgnews_pic') -> where(array('id' => $id)) -> save(array('status' => 0));
		if($result){
			$this -> writelog("已删除id为{$id}的手游资讯图","bbs_mgnews_pic",$id,__ACTION__ ,"","del");
			$this -> assign("jumpUrl","/index.php/Bbs_manage/Mgnews/mgnews_list".$where_go);
			$this -> success("删除成功");
		}else{
			$this -> error("删除失败");
