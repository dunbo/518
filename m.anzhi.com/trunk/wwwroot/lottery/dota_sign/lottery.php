...
//检查此礼券是否领取过
$kind_award_list = get_user_kind_award_list($uid,$active_id,'dota_sign','valentine_draw_award');
...
//通过包名获取软件名称
$soft_info	=	get_soft_info('sh.lilith.dgame.anzhi');
...
if($lottery_rs['code'] == 1){	
	...
	$redis -> set($award_info_key, $lottery_rs, 30*86400);
	//设置已签到状态
	$tm_con[date('Y-m-d',$time)]['status'] = 1;
	$redis -> set("{$prefix}:{$active_id}_tm_config:".$uid, $tm_con, 30*86400);
	//增加签到数
