...
$login_url = $center_url.$activity_host."/lottery/{$prefix}/2017_03_index.php?".$build_query;
...
$tplObj -> out['is_lottery'] = $redis->get($imei_key);
//获取礼包礼券id---过滤礼包礼券
$pid_arr = get_virtual_pid($active_id,$prefix,'valentine_draw_prize');
$where = array(	
	'pid' => array('exp',"not in (".$pid_arr.")"),
	'status' => 1,
	'aid' => $active_id,
);
$award_all = get_award_all_new($active_id, $prefix, 'valentine_draw_award',$where);
$tplObj -> out['award_all'] = $award_all;//获取获奖人信息奖品
//奖品展示
$tplObj -> out['prize_list'] = get_prize_list();
if($_GET['stop'] == 1){
	$tpl = "lottery/{$prefix}/2017_03_end.html";
}else{
	//指定用户中奖
	add_test_user_award();
	$tpl = "lottery/{$prefix}/2017_03_index.html";
}
