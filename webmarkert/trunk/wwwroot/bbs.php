...
$type = $_GET['type'];
$type_array = array('gift_list','my_gift','gift_detail','game_index','giftlist_game','newservice','splash');
...
switch($type){
	...
	case 'splash':
		$_SESSION['MODEL_CID'] = $_GET['cid'];
		$_SESSION['product'] = 11;
		$result = gomarket_action('market.GoGetSplashScreen',$param);
		unset($_SESSION['MODEL_CID']);
		unset($_SESSION['product']);
	break;
}
if ($_GET['action']) $result['DATATYPE'] = $_GET['action'];
$method = isset($_GET['method']) ? $_GET['method'] : 'jsonp';
