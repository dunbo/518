...
include_once(dirname(realpath(__FILE__)) . "/christmas2015_init.php");

// 判断是否有imsi
if ($imsi_status != 1) {
	$tplObj->display('lottery/christmas2015/christmas2015_no_sim.html');
	exit;
}
...
if ($tplObj->out['check_status'] != 1 && !$jump_free) {
	...
	// 跳转到提示升级页面
	$tplObj->display("lottery/christmas2015/christmas2015_update_hint.html");
	exit;
}