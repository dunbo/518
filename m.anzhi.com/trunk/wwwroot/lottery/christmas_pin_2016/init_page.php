...
if ($imsi_status != 1&&!$_GET['is_share']) {
	$tplObj->display('lottery/christmas_pin_2016/no_sim.html');
	exit;
}
...
//$_SESSION['VERSION_CODE'] = 6300;  //测试用的
if ($_SESSION['VERSION_CODE'] < 6000&&!$_GET['is_share']) {
	...
	if ($alone_update) {
		...
		if (!empty($channel_result)) {
			$check_status = 3;// 可以升级的 少年快去升级安智市场，再来查看活动吧！
		} else {
			$check_status = 4;// 不可以升级 快去升级安智市场再来查看活动吧~
		}
	} else {
...
if ($tplObj->out['check_status'] != 1 && !$_GET['is_share']) {
	// 记日志
	$log_data = array(
		'imsi' => $imsi,
	...
	// 跳转到提示升级页面
	$tplObj->display("lottery/christmas_pin_2016/update_hint.html");
	exit;
}