...
class AdCustomerAction extends CommonAction{
	function index() {
		$model = D('Channel_cooperation.AdCustomer');
	...
	function editdo() {
		...
		if($re == false){
			$this -> assign('jumpUrl','/index.php/Channel_cooperation/AdCustomer/index');
			$this->error("您未做修改");
	...
	//转移操作
	function transfers(){
		$models = D('Channel_cooperation.AdCustomer');
		$model = new Model();
		$check = $_POST['check'];
