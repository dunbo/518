...
$worker->addFunction("activity_video_change_format", "process_video");
while ($worker->work());
...
//			$proc = proc_open($cmd,$descriptorspec, $pipes);
			...
			while(true) {
				...
				usleep(500);
				if ($n ==0) {
					writelog("a_video_change_format.log",filesize($h263_url_tmp));
					$c_cmd = "cp {$h263_url_tmp} {$h263_url}";
					shell_exec($c_cmd);
				}
				if (file_exists($h263_url)&&$n==0) {
					unlink($h263_url_tmp);
					writelog("a_video_change_format.log",$h263_url);
					...
					if($process_tm>60&&$process_tm<600){
						if($n==0){
							writelog("a_video_change_format.log",$cmd."视频未转码成功，进程异常退出");
							break;
						} 
					}else if($process_tm>600){
						writelog("a_video_change_format.log",$cmd."已处理10分钟自动退出");
						break;
					}
				}
			}
...
function save_url($data,$url){
	...
	if($data['type'] == 2){
		$u_data['marquee_text_color'] = $url;
	}else{
		$u_data['nextpage'] = $url;
	}
	//if(!empty($screen_mode)) $u_data['screen_mode'] = $screen_mode;
	$model->update($where,$u_data,'master');
	writelog("a_video_change_format.log",$model->getSql());
}
