...
class UpdatePackageWebsiteConfigAction extends CommonAction {
    ...
    public function index() {
        ...
        $list = $model->table('cj_update_website_config')->where($where)->order('status, priority asc')->select();
    ...
    public function toggle_enabled_status() {
        ...
        $model = D('Caiji.UpdatePackageWebsiteConfig');
        $ret = $model->table('cj_update_website_config')->where($where)->save($data);
        if ($ret) {
            $this->writelog("采集/质管--配置管理--更新网站配置：{$hint}了id为{$id}的记录",'cj_update_website_config',$id,__ACTION__ ,"","edit");
    ...
    public function delete_website() {
        ...
        $ret = $model->table('cj_update_website_config')->where($where)->save($data);
        if ($ret) {
            $this->writelog("采集/质管--配置管理--更新网站配置：删除了id为{$id}的记录",'cj_update_website_config',$id,__ACTION__ ,"","del");
            $this->success("删除成功！");
        } else {
            $this->error("删除失败！");
        }
    ...
    public function add_website() {
        if ($_POST) {
            ...
            $get_detail_type = trim($_POST['get_detail_type']);
            if (!$get_detail_type) {
                $this->error("获得详情页方式不能为空！");
            }
            $data['get_detail_type'] = $get_detail_type;
            
            if ($get_detail_type == 1) {
                $search_url = trim($_POST['search_url']);
                if (!$search_url) {
                    $this->error("搜索url不能为空！");
                }
                ...
                $detail_url_fake = trim($_POST['detail_url_fake']);
                if (!$detail_url_fake) {
                    $this->error("请填写替换url");
                }
                $data['detail_url_fake'] = $detail_url_fake;
            }
            ...
            $model = D('Caiji.UpdatePackageWebsiteConfig');
            $ret = $model->table('cj_update_website_config')->add($data);
            if ($ret) {
                // 写日志
                $this->writelog("采集/质管--配置管理--更新网站配置：添加了id为{$ret}的记录",'cj_update_website_config',$ret,__ACTION__ ,"","add");
    ...
    public function edit_website() {
        if ($_POST) {
            ...
            $where = array('id'=>$id);
            // 编辑前记录信息
            $log = $this->logcheck($where, 'cj_update_website_config', $data, $model);
            $ret = $model->table('cj_update_website_config')->where($where)->save($data);
            if ($ret) {
                // 写日志
                $this->writelog("采集/质管--配置管理--更新网站配置，编辑了id为{$id}的记录：{$log}",'cj_update_website_config',$id,__ACTION__ ,"","edit");
                $this->assign('jumpUrl', '__URL__/index');
                $this->success("编辑成功！");
            } else {
                $this->error("编辑失败！");
            ...
            $find = $model->table('cj_update_website_config')->where($where)->find();
