...
$page_id = $_GET['ap_id'] ? $_GET['ap_id'] : $result['activity_page_id']; //活动页面id，用来关联软件
$pre_page_id = $_GET['ap_id'] ? $_GET['ap_id'] : $result['activity_prepare_id']; //预告页面id
$end_page_id = $_GET['ap_id'] ? $_GET['ap_id'] : $result['activity_end_id']; //结束页面id
$a_name = $result['name']; //活动名称
$a_pre_url = $result['pre_url']; //活动预告页面
