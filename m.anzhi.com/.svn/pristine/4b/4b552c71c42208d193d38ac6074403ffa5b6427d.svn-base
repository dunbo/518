...
if($_GET['flow_up']){
	...
	$tpl = "lottery/".$prefix."/flow_up".$from_type.".html";		
}else if($_GET['flow_extract_succ'] == 1){
	$tplObj -> out['price'] =$_GET['price'];
	$tplObj -> out['activity_host'] = $configs['activity_url'];	
	$tpl = "lottery/".$prefix."/result".$from_type.".html";	
}else if($_GET['is_rule'] == 1){
	if($active_id == 1226){
		$tpl = "lottery/".$prefix."/rules3.html";	
	}else{	
		$tpl = "lottery/".$prefix."/rules".$from_type.".html";	
	}
}else{
	//记录页
