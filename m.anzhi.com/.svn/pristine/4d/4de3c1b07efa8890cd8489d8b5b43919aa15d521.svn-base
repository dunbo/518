...
$url = "http://promotion.anzhi.com/lottery/ghost/index.php?".$build_query;
...
if($_POST){
	...
	if($_GET['types'] == 1){
		//我的奖品
		$kind_award_list = get_user_kind_award_new($uid,$active_id,'ghost','valentine_draw_award');
		$tplObj -> out['kind_award_arr'] = $kind_award_list;
		$kind_award_gift = get_user_kind_gift_new($uid,$active_id,"ghost","valentine_draw_gift");
	...
	//用户信息
	$userinfo = get_user_info_new($uid,$active_id,"ghost","valentine_draw_userinfo");
	...
	$tplObj -> display('lottery/ghost/userinfo.html');
}
