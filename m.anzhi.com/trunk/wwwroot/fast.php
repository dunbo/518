...
switch ($type) {
    ...
    case 'feature':
        $flag = 12;
        $second_value = $_GET['id'];
//{"SUBJECT_ID":"专题id","TYPE":1或者2（1：端页面，2：网页）,"TITLE":"标题","URL":"专题url","PKG_NAME":"包名"}
        $model = new GoModel();
        $option = array(
            'table' => 'sj_feature',
            'feature_id' => $_GET['id'],
        );
        $row = $model->findOne($option);
        $arr = array(
            'SUBJECT_ID' => $_GET['id'],
            'TYPE' => '1',
            'TITLE' => $row['name'],
        ...
        $extent_str = json_encode($arr);
    break;  

    case 'content':
        $flag = 11;
        $second_value = $_GET['id'];
        $extent_str = isset($_GET['pkg']) ? $_GET['pkg'] : '';
    break;  
