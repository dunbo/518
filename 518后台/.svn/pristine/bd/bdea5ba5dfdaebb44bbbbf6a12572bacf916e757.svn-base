...
class Page extends Think {
    ...
    protected $rollPage   ;
	// 分页显示定制
    protected $config  =	array('header'=>'条记录','prev'=>'上一页','next'=>'下一页','first'=>'第一页','last'=>'最后一页','theme'=>' %totalRow% %header% %nowPage%/%totalPage% 页 %upPage% %downPage% %first%  %prePage%  %linkPage%  %nextPage% %end% %jumpPage% ');
    ...
    public function show() {
        ...
        if($nowCoolPage == $this->coolPages){
            ...
            $nextPage = "<a href='".$url."&".$p."=$nextRow&lr=".$lr."' >下".$this->rollPage."页</a>";
		...
		$jumpPage  =  $jumpjs."&nbsp;<form name='form1' id='pageForm' method='get' {$onsubmit}>";
		$jumpPage .=  "&nbsp;每页显示记录数(最大输入1000条，最少10条)&nbsp;<input id='listRows' name='lr' size='4' value='".$lr."' onkeydown='onlyNum();'  />";
		$jumpPage .=  "&nbsp;跳转至第&nbsp;<input id='jumpPage' name='p' onkeydown='onlyNum()' type='text' value=".$this->nowPage." size='4'  />&nbsp;页";
		$jumpPage .=  "&nbsp;<input type='submit' value='Go' alt='跳转' title='跳转' /></form>";
