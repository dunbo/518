...
class ChannelsettleAction extends CommonAction{
	// 商务确认列表
	...
	function edit_salvation(){
		...
		if($result){
			...
			$this -> assign('jumpUrl','/index.php/Channel_cooperation/Channelsettle/business_list'.$where_go);
			$this -> success("编辑成功");
		}else{
			$this -> error("编辑失败");
		}
	...
	function edit_comment(){
		...
		if($result){
			...
			if($_GET['from'] == 2){
				$this -> assign('jumpUrl','/index.php/Channel_cooperation/Channelsettle/financial_settle'.$where_go);
			}elseif($_GET['from'] == 4){
				$this -> assign('jumpUrl','/index.php/Channel_cooperation/Channellist/settle_list'.$where_go);
			}elseif($_GET['from'] == 5){
				$this -> assign('jumpUrl','/index.php/Channel_cooperation/Channelsettle/business_list'.$where_go);
			}elseif($_GET['from'] == 99){
					$this -> assign('jumpUrl','/index.php/Channel_cooperation/Channelsettle/forum_settle'.$where_go);
			}else{
				$this -> assign('jumpUrl','/index.php/Channel_cooperation/Channelsettle/operation_check_list'.$where_go);
			}
			$this -> success("编辑成功");
		}else{
			$this -> error("编辑失败");
	...
	function financial_settle(){
		...
		foreach($result as $key => $val){
			...
			if($settle_entity_status == 1){
				...
				$the_amount_pay_arr[] = $all_amount_pay_arr[0];
				$val['bill_amount'] = array_sum($bill_amount_new)/count($bill_amount_new);
		...
		//file_put_contents('/tmp/bill_amount.txt', json_encode($all_bill_amount_arr));
	...
	public function forum_settling()
	{
		...
		$cc_model = D('Channel_cooperation.channel_cooperation');
		$model = new Model();
		...
		if($this->isPost())
		{
			...
			if($result){
				...
				$this -> assign('jumpUrl','/index.php/Channel_cooperation/Channelsettle/forum_settle');
				$this -> success("编辑成功");
			}else{
				$this -> error("编辑失败");
			}
