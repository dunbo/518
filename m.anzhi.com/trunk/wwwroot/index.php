...
if (!strstr ( TEMPLATE_DIR, "uc" )) {
    ...
    $_SESSION['VERSION_CODE'] = 4000;
    $ads = gomarket_action('v53.GoGetRecommend', array('VR' => 4,'KEY' => 'GET_RECOMMEND_NEW'));
    $adlist = array();
	...
	if ($_GET['type'] == 'new'){
		...
		if ($channel == 'bbg' && $_GET['type'] == '' && $resultanzhi != '' )
		{
			if ($morelist == $bbg_page)
			{
				$result = get_softlist ( 'softv2.GoGetExtentHomeFeatureOld', $morelist * $re_size, $re_size - 1);
			}
			elseif ($morelist > $bbg_page)
			{
				$result = get_softlist ( 'softv2.GoGetExtentHomeFeatureOld', $morelist * $re_size - 1, $re_size);
			}
			else
			{
				$result = get_softlist ( 'softv2.GoGetExtentHomeFeatureOld', $morelist * $re_size, $re_size);
			}
		}
		else
		{
