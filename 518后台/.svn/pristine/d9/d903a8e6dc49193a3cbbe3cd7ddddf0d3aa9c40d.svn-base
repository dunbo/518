...
class NewFeatureLabelManagementAction extends CommonAction {
    protected $table = 'new_feature_label_management';
    private $max_rank = 20;
    ...
    public function batch_rank() {
        ...
        $this->writelog("论坛PC端管理-新精品资源-标签管理：专题标签批量编辑排序，{$log}");
    ...
    public function add() {
        if ($_POST) {
            ...
            if ($ret) {
                $this->writelog("论坛PC端管理-新精品资源-标签管理：专题标签新增了id为{$id}的专题标签");
    ...
    public function edit() {
        ...
        if ($_GET['id']) {
            ...
            $ret = $bbs_model->table($this->table)->where($where)->save($data);
            if ($ret) {
                $this->writelog("论坛PC端管理-新精品资源-标签管理：专题标签编辑了id为{$id}的专题标签，{$log_result}");
    ...
    public function del() {
        if ($_GET['id']) {
            ...
            $ret = $bbs_model->table($this->table)->where($where)->save($data);
            if ($ret) {
                $this->writelog("论坛PC端管理-新精品资源-标签管理：专题标签删除了id为{$id}的专题标签");
                $this->success("删除成功");
            } else {
                $this->error("删除失败");
            }
