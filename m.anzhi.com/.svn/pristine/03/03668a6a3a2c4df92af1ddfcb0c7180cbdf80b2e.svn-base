<?php
class hotSearchList extends Action{
    public $params;
    public $device;
	...
	public function getData()
	{
        ...
        $pagesize = isset($this -> params['limit']) ? $this -> params['limit'] : 10;
         $key       = addslashes($this -> params['keyword']);
         ...
         foreach ($sorted_fields as $label => $field){
             if (strpos($fields, $label . "") !== false){
                 ...
                 while($app = $softObj -> fetch($result)){
                     ...
                     $file['iconurl'] = STATIC_IMG_HOST.$file['iconurl'];//load_config('full_static_host') . $file['iconurl'];
