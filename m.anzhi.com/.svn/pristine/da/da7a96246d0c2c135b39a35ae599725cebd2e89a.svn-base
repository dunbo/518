...
session_begin($sid);
$url = "http://promotion.anzhi.com/lottery/ghost_sign_lottery/index.php?aid={$active_id}&sid={$sid}";
...
if($tm_con[date('Y-m-d',$time)]['num']){	
	...
	$redis->set("ghost_sign_lottery:{$active_id}_tm_config:".$uid,$tm_con,86400*10);
}
$i = 0;
