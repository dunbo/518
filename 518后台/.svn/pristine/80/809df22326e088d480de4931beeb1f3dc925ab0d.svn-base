 ...
 * ----------------------------------------------------------------------------
*/
class WebNewAdminAction extends CommonAction {
    private $uid;           //类型id
    ...
    public function admindel() {
		...
		if(!$this->login_pwd_chk()) {
			...
			$this->assign('jumpUrl','/index.php/'.GROUP_NAME.'/WebNewAdmin/index');
    ...
    public function adminuseradd() {
		...
		if(!$this->login_pwd_chk()) {
			...
			$this->assign('jumpUrl','/index.php/'.GROUP_NAME.'/WebNewAdmin/adminuseradd');
			$this->error('密码验证失败，操作失败！');
		}
		//登录管理员密码验证,结束
        $this->adminuser_db=M('admin_users');
		...
		$this->adminuserlist['type'] = 1;
		$this->adminuserlist['plain_password'] = $this->encrypt($_POST['admin_user_password']);
		if (!empty($_POST['admin_user_email'])) {
			$this->adminuserlist['admin_user_email'] = $_POST['admin_user_email'];
		}
        if(empty($this->adminuserlist['admin_user_name']) || empty($_POST['admin_user_password'])  ) {
            $this->assign('jumpUrl','/index.php/'.GROUP_NAME.'/WebNewAdmin/adminadd');
    ...
    public function upadmincommision() {
		...
		if(0 && !$this->login_pwd_chk('_login_password_ag_com')) {
			...
			$this->assign('jumpUrl','/index.php/'.GROUP_NAME.'/WebNewAdmin/admincommision/id/'.$_GET['id']);
			$this->error('密码验证失败，操作失败！');
		}
		//登录管理员密码验证,结束
        ...
        if($text=='')
        {
            //$this->assign('jumpUrl','/index.php/'.GROUP_NAME.'/WebNewAdmin/admingroup_list');
    ...
    public function admingroupadd() {
		...
		if(!$this->login_pwd_chk('_login_password_aga')) {
			...
			$this->assign('jumpUrl','/index.php/'.GROUP_NAME.'/WebNewAdmin/admingroup_add');
			$this->error('密码验证失败，操作失败！');
		}
		//登录管理员密码验证,结束
        $this->admingroup_db=M('admin_group');
    ...
    public function admingroupedit_edit() {
		...
		if(!$this->login_pwd_chk('_login_password_agedit')) {
			...
			$this->assign('jumpUrl','/index.php/'.GROUP_NAME.'/WebNewAdmin/admingroupedit/id/'.$_GET['id']);
			$this->error('密码验证失败，操作失败！');
		}
		//登录管理员密码验证,结束
    ...
    public function adminusercommision() {         //权限列表_显示
		...
		$subQuery = $adminrole_db->where($where_node)->field('node_id')->buildSql();
		
		$node_list_result = $model
			->Table('sj_admin_node B, sj_admin_note_group AS C')
			->where("C.node_id in ({$subQuery}) and C.node_id=B.node_id")
			->field('B.node_id, B.nodename, B.postil, B.type, B.note, C.group_id')
			->select();
		...
		$groupid_arr = array();
		foreach ($node_list_result as $node) {
			$groupid_arr[] = $node['group_id'];
		}
		$where = array(
			'group_id'=>array("in",$groupid_arr)
		);
		$node_group_list_result = $model->Table('sj_admin_node_group')->where($where)->select();
		$node_group_list = array();
        foreach($node_group_list_result as $key => $value)     // 循环判断该类别是否被选中
        {
        	$group_id = $value['group_id'];
    ...
    public function softReplaceLog(){
    	...
    	if(empty($_GET['p'])) {
			...
			$_SESSION['WebNewAdmin']['softReplaceLog']['params'] = array('time'=>$time,'user'=>$user, 'id' => $sid);
    	}else{
    		$time = $_SESSION['WebNewAdmin']['softReplaceLog']['params']['time'];
			$from_value = $time[0];
			$to_value = $time[count($time)-1];
			$user = $_SESSION['WebNewAdmin']['softReplaceLog']['params']['user'];
			$sid = $_SESSION['WebNewAdmin']['softReplaceLog']['params']['id'];
    	}
...
//			}
	...
	function alllog()
	{
        ...
        $model = new Model();
        $sql = "SELECT * FROM sj_admin_users WHERE admin_state = 1 and admin_user_id <> 1 and type=1";
		$admin_users_d = $model->query($sql);
		$admin_users = array();
		$aids = array();
		foreach ($admin_users_d as $a){
		...
		$subQuery = $model->table('sj_admin_role')->where($where_node)->field('node_id')->buildSql();
		if(!empty($map)) {		
        	$map['action_id'] = array('in',$subQuery);
        	if (!isset($map['admin_id'])) $map['admin_id'] = array('in',$aids);
        }
        ...
        for($i=0;$i<count($this->adminloglist);$i++)
        {
            ...
            {
				 $this->adminloglist[$i]['admin_name']= $admin_name_arr[$this->adminloglist[$i]['admin_id']] ? $admin_name_arr[$this->adminloglist[$i]['admin_id']] : $this->adminuser_db->where('admin_user_id='.$this->adminloglist[$i]['admin_id'].'')->getField('admin_user_name');
            }
             //管理员名称
            if(empty($this->adminloglist[$i]['action_id'])) {
                $this->adminloglist[$i]['action_id']='-';
        ...
        $this->assign ("page", $show );
        $this->assign('adminloglist',$this->adminloglist);
        $this->display('Admin:WebAdmin:alllog');
	}
	...
	function adminusercommision_csv(){
        ...
        header('Content-Disposition: attachment;filename="WebNewAdmin.csv"');
	...
	function login_qa_edit() {
		...
		if(!$this->login_pwd_chk()) {
			...
			$this->assign('jumpUrl','/index.php/'.GROUP_NAME.'/WebNewAdmin/login_qa');
			$this->error('密码验证失败，操作失败！');
		}
		//登录管理员密码验证,结束
