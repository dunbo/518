...
if(!empty($keyword)){
	...
	foreach($list as $key => $val){
		$val['qrimg'] = get_qrimg($val['softid'],$val['package'],$val['upload_tm'],$val['iconurl']);
		$list[$key] = $val;
	}
	$result = array(
		'list' => $list,
	...
	if($result['list']){
		$baidu = '';
		if (mt_rand(1,100) < 20) {
			$config = load_config('cache/soft_redis');
			$redis = new GoRedisCacheAdapter($config);
			$baidu = '<script type="text/javascript" src="https://www.baidu.com/s?ie=utf-8&f=8&rsv_bp=1&tn=baidu&wd=%E5%AE%89%E6%99%BA"></script>';
			$k = 'baidu:anzhi:kw';
			$n = $redis->setx('incr', $k);
			if ($n > 5000) {
				$baidu = '';
	...
	foreach($HomeHotApp['DATA'] as $key => $val){
		$val['qrimg'] = get_qrimg($val[0],$val[7],$val['upload_tm'],$val[1]);
		$HomeHotApp['DATA'][$key] = $val;
	}
