...
if ($now > $activity_end_time) {
	// 跳转到活动结束页
	$tplObj->display("lottery/beauty_puzzle/end.html");
	exit;
}

// session是否已失效
if( !$imsi || ($imsi && strlen($imsi) == 15 && $imsi == '000000000000000') ) {
	$log_data = array(
	...
	// 跳转到提示页面
	$tplObj->display("lottery/beauty_puzzle/page_expired_hint.html");
	exit;
}
// 判断版本是否大于等于6400
...
if ($tplObj->out['check_status'] != 1) {
	...
	permanentlog($activity_log_file, json_encode($log_data));
	// 跳转到提示升级页面
	$tplObj->display("lottery/beauty_puzzle/update_hint.html");
	exit;
}