	...
	if($_POST){
		...
		if($rest_integral <= 0){
			exit(json_encode(array('code'=>0,'msg'=>'您当前积分不足，请您充值后再抢夺')));
                ...
                if($_POST['position']==4){
                        if($_POST['num']>1){
                                    exit(json_encode(array('code'=>0,'msg'=>'该奖品每人限兑1次')));
                        }
                	$kind_award_gift = get_user_kind_gift_new($uid,$active_id,"{$prefix}","valentine_draw_gift");
                        if($kind_award_gift!=false){
			    exit(json_encode(array('code'=>0,'msg'=>'该奖品每人限兑1次,您已经兑过了哦')));
                        }
                }
                ...
                if($_POST['position']==8){
                        ...
                        foreach($kind_award_list as $v){
                            if($v['position']==8){
                                if(($v['integral']+$_POST['num'])>5){
                                    exit(json_encode(array('code'=>0,'msg'=>'该奖品每人限兑5张,您已经兑换够了哦')));
                                }
                            }
                        }
                }
		...
		$new_array = array(
			...
			'pid' => $prize_info['id'],//奖品ID
			'pic_name' => $prize_info['pic_name'],
			'prize_integral' => $prize_info['prize_integral'],//所需积分
			'prizename' => $prize_info['prizename'],
			'num' => $_POST['num'],
			'position' => $_POST['position'],
		);
		$the_award = $task_client->do("integral_2016_worker", json_encode($new_array));
