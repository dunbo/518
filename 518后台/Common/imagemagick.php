...
/* 
 * params @图片信息 $param type array();
 * return array(width,height,是否压缩 0/1)
 */
function thumb_by_imagemagick_1($param)
{
        ...
        $to = $param['to'];
        $width = $param['width']; //压缩后的宽
        $height = $param['height']; //压缩后的高
        $height = is_null($height) ? 0 : $height;
        $filesize = $param['filesize']; //原图大小
        $resize_w = $param['resize_w']; //是否裁宽
