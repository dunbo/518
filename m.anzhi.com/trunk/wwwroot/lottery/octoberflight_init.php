...
$rkey_imsi_open_info = "octoberflight:{$aid}:{$imsi}:open_info";//用户打开此活动天数信息，hash类型
$rkey_imsi_question_answer = "octoberflight:{$aid}:{$imsi}:question_answer";//用户回忆问题答案，hash类型
$rkey_imsi_share_info = "octoberflight:{$aid}:{$imsi}:share_info";//用户最后一次分享日期，字符串类型
$rkey_imsi_lottery_num = "octoberflight:{$aid}:{$imsi}:lottery_num";//用户可抽奖次数
$rkey_imsi_package_list = "octoberflight:{$aid}:{$imsi}:package_list";
...
function get_lottery_num() {
	...
	if (empty($now_num) && $now_num !== 0) {
		...
		$option = array(
			...
			'table' => 'octoberflight_lottery_num',
		);
		$find = $model->findOne($option, 'lottery/lottery');
		if (empty($find)) {
			$now_num = 0;
			$ret = $model->query("insert into `octoberflight_lottery_num` (`imsi`, `lottery_num`, `time`) values ('{$imsi}', {$now_num}, {$now}) ON DUPLICATE KEY UPDATE `lottery_num`={$now_num};", 'lottery/lottery');
		} else {
...
function set_lottery_num($now_num) {
	...
	$data = array(
		'lottery_num' => $now_num,
		'time' => $now,
		'__user_table' => 'octoberflight_lottery_num',
	);
