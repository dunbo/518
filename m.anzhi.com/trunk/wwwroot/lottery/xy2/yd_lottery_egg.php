...
if($_POST){
	...
	//剩余抽奖次数
        if($type==1){
            $lottery_num_key = ($_POST['eggtype']==1)?'yd:gold:lotterynum:uid:'.$uid:'yd:silver:lotterynum:uid:'.$uid;
        }else if($type==2){
	...
	exit(json_encode($array));
        ...
        $za_num = $redis->get($gold_user_lotterynum_key)+$redis->get($silver_user_lotterynum_key);
        $za_num = empty($za_num)?0:$za_num;
        ...
        if(empty($prizeuser)){
                $lunbo = '活动已开启，快去砸蛋吧！中奖率100%哦！';
        }else{
            foreach($prizeuser as $v){
                $lunbo = $lunbo.'恭喜用户'.str_replace_cn_new($v['username'],1,-2).',好运爆表,砸蛋获得'.$v['prizename'].';';
            }
        }
	...
	$tplObj -> display('lottery/xy2/yd_lottery_egg.html');	
}
