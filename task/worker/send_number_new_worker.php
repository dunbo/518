...
$worker->addFunction("send_number_new" . $suffix, "send_number_func");
...
function send_number_func($jobs) {
	...
	if (!empty($num)) {
		...
		$redis->pingConn();
		$redis->set($key, $v, 88400);
		$redis->sethash($user_key, array($active_id => array('code' => $num, 'status' => 1)));
		...
		if($uid){
			...
			$gift_data = array(
				...
				'giftQty' => 1,//数量
				'memo' => '',//备注
				'prizeFrom' => 3,//数据来源 1 :抽奖  2：兑换  3：领取 
				'secrects' => $num,//卡密
...
function get_olgame_info($active_id) {
	...
	$where = array(
		'A.apk_name' => $result['apply_pkg'],
	);
	$option = array(
		'where' => $where,
		'table' => 'sj_soft_file AS A',
		'field' => 'A.iconurl,B.softname',
		'join' => array(
			'sj_soft AS B' => array(
				'on' => array('A.apk_name', 'B.package')
			)
		),
		'order' => 'A.softid desc'
	);
...
function decrease_coin($gift_id, $uid, $coin, $giftcode) {
	...
	$ch = curl_init();
	$url = 'http://bbs.anzhi.com/api/gift.php?action=delcredit';
