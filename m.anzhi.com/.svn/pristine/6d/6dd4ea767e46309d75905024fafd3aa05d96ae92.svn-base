...
$fix_package = '["fw.cn.quanmin","com.husor.beibei","com.meelive.ingkee","com.tencent.qqlive"]';
...
$rkey_imsi_puzzle = "long_holiday_2016:{$aid}:{$imsi}:".$date;//用户每天拼图次数
$rkey_imsi_lottery_num = "long_holiday_2016:{$aid}:{$imsi}:rest_lottery_num";//用户可抽奖次数
$rkey_imsi_package_list = "long_holiday_2016:{$aid}:{$imsi}:package_list"; //下载的包
$rkey_imsi_pics_list = "long_holiday_2016:{$aid}:{$imsi}:pics_list"; //图片
...
function get_pics($aid){
	...
	if($puzzle_pics === null){	
		...
		$puzzle_pics = json_decode($puzzle_result['pics'],true);
		
		$redis->set($rkey_imsi_pics_list,array_values($puzzle_pics),10*86400);
	}
	return $puzzle_pics;			
}
//根据获奖等级得到奖品名称
