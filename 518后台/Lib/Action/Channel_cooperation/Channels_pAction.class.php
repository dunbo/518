 ...
 * ----------------------------------------------------------------------------
*/
class Channels_pAction extends CommonAction {
	...
	public function channels_coefficient_list(){
		...
		if($_GET['co_status']){
			...
			$coefficient_result = $Form -> table('sj_channel_coefficient_p') -> where($coefficient_where) -> select();
		...
		foreach($list as $key => $val){
			...
			$cofficient_result = $Form -> table('sj_channel_coefficient_p') -> where(array('cid' => $val['cid'])) -> select();
			$val['cofficient'] = $cofficient_result[0]['cofficient'];
			$val['last_refresh'] = $cofficient_result[0]['last_refresh'];
			$list[$key] = $val;
		}
		$my_cid_result = $Form -> table('sj_channel_coefficient_p') -> group('cid') -> select();
		foreach($my_cid_result as $key => $val){
			$my_cid[] = $val['cid'];
		}
        ...
        {
            $cid = $v['cid'];
            $sql = "select coefficient,create_time from sj_channel_coefficient_p where cid={$cid} and status=1" ;
	...
	public function channels_coefficient_edit(){
        ...
        $this->assign( "show_chl", $show_chl );
		if(empty($cid)){
			$this->assign('jumpUrl','/index.php/'.GROUP_NAME.'/Channels_p/channels_coefficient_list');
		...
		$sql = "select * from sj_channel_coefficient_p where cid={$cid} and status=1";
	...
	public function channels_coefficient_update(){
		...
		if(!empty($old)){
			if($old[0]['coefficient']!=$coefficient){
				$sql = "update sj_channel_coefficient_p set last_refresh =".$t." ,status=0 where cid={$cid}";
				$model->query($sql);
				$sql = "insert into sj_channel_coefficient_p (cid,coefficient,create_time,status,last_refresh) values ($cid,$coefficient,$t,1,$t)";
				$model->query($sql);
			}
		}else{
			$sql = "insert into sj_channel_coefficient_p (cid,coefficient,create_time,status,last_refresh) values ($cid,$coefficient,$t,1,$t)";
		...
		if($list!==false){
			...
			$this->assign('jumpUrl','/index.php/'.GROUP_NAME."/Channels_p/channels_coefficient_list/p/{$p}/lr/{$lr}".$where_go);
			$this->success('数据更新成功');
		}else{
			$this->assign('jumpUrl','/index.php/'.GROUP_NAME."/Channels_p/channels_coefficient_list/p/{$_GET['p']}/lr/{$_GET['lr']}".$where_go);
			$this->error('数据更新失败');
		}
	}
	...
	function coefficient_history_list(){
		...
		if(strtotime($end_tm) > strtotime(date('Y-m-d',time() - 86400))){
			$end_tm = date('Y-m-d 00:00:00',time() - 2*86400);
		}
		...
		if($from == 1){
			...
			//echo $coefficient_model->getLastSql();exit;
	...
	function coefficient_stats(){
		...
		if(!$_GET['my_show'] || $_GET['my_show'] == 1){
			...
			$all_coefficient_result = $model -> table('sj_channel_coefficient_p') -> where(array('status' => 1)) -> select();
			...
			foreach($the_channel_result as $key => $val){
				...
				$the_channel_coefficient_result = $model -> table('sj_channel_coefficient_p') -> where(array('cid' => $val['cid'],'status' => 1)) -> select();
	...
	function activatequantity(){
		$channel_activation = D('Channel_cooperation.channel_coefficient_p');
		$model = new Model();
		$cid = $_GET['cid'];
		if(!$_GET['my_time']){
			$my_time = 2;
