...
if(!$sid){
	...
	$tpl = "lottery/wzcq/index2_web.html";	
}else{
	$tpl = "lottery/wzcq/index2.html";	
}
$tplObj -> display($tpl);
function get_soft_info($wzcq_pkg,$is_test){
	global $model,$redis,$prefix,$active_id,$configs;
	$soft_info_key = $prefix.":".$active_id.':soft_info';
	$soft_info = $redis->get($soft_info_key);	
	if($soft_info === null){
		$img_dir = $configs['new_static_url'].'/activity/'.$prefix.'/images2/';
		if($is_test){
			...
			$icon_arr = array(
				'cn.jj' => $img_dir.'app_icon1.png',
				'com.netease.l10.anzhi' => $img_dir.'app_icon2.png',
				'mgyly1.anzhi' => $img_dir."app_icon3.png",		
			);			
		}else{
			...
			$icon_arr = array(
				'com.tencent.tmgp.sgame' => $img_dir.'app_icon1.png',
				'com.netease.l10.anzhi' => $img_dir.'app_icon2.png',
				'com.pokercity.bydrqp.anzhi' => $img_dir.'app_icon3.png',		
			);
		}
