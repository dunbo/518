<?php
class FlowWhiteListAction extends CommonAction {
	
	public function flow_white_list() 
	{
		$model = M('flow_white_list');
		$count = $model -> table('sj_flow_white_list')  -> count();
		import("@.ORG.Page");
		$param = http_build_query($_GET);
		$Page = new Page($count, 20, $param);
		$result = $model -> table('sj_flow_white_list') -> limit($Page->firstRow . ',' . $Page->listRows) -> order('create_tm DESC') -> select();
		if($_GET['lr']){
			$lr = $_GET['lr'];
		}else{
			$lr = 20;
	...
	public function flow_white_add() 
	{
		$model = M('flow_white_list');
		if ($_FILES) 
		{
            ...
            // 判断后台有没有人正在导入
            $lock_name = 'sj_flow_white_package';
            ...
            $save_name = 'flow_white_list_'. time() . '.csv';
			...
			if ($file_id) 
			{
				$this->writelog('流量白名单_添加文件'.$save_name,'sj_flow_white_list',$file_id,__ACTION__ ,'','add');
    ...
    function process_import_array_soft_package($content_arr,$file_id) {
        $result_arr = array();
        $model = M('flow_white_package');
		//添加之前先把之前表中数据删除完
		//$del_sql="TRUNCATE sj_flow_white_package";
		$del_sql = "UPDATE `sj_flow_white_package` SET status = 0";
        ...
        foreach($content_arr as $key => $record) 
		{
			...
			if ($id) 
			{
				$this->writelog('流量白名单_添加包名'.$record['package'],'sj_flow_white_package',$id,__ACTION__ ,'','add');
