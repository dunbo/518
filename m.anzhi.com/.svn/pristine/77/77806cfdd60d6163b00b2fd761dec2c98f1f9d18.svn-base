...
if($_GET['stop'] != 1 && ($active_id == 365 || $active_id == 393)){
	$res = activity_is_stop($active_id);
	if(!$res){
		$url = $center_url."http://promotion.anzhi.com/lottery/nov_sign/index.php?stop=1&aid=".$active_id."&sid=".$sid;
		header("Location: {$url}");
	}
}
//获取活动配置
function get_config($aid,$uid){
	global $model;
	global $redis;	
	$tm_config = $redis->get("nov:{$aid}_tm_config:".$uid);	
	if($tm_config === null){	
		$option = array(
			'where' => array(
				'id' => $aid,
		...
		$redis->set("nov:{$aid}_tm_config:".$uid,$tm_config,10*86400);
	}
	return $tm_config;			
}
//获取抽奖次数
function get_lottery_num($aid,$uid){
	global $model;
	global $redis;		
	$lottery_num = $redis->get("nov:{$aid}_rest_lottery_num:".$uid);
	if($lottery_num === null){
		$option = array(
			'where' => array(
				'uid' => $uid,
		...
		$redis->set("nov:{$aid}_rest_lottery_num:".$uid,intval($lottery_num),10*86400);		
	}	
	return $lottery_num;		
}
//添加用户、修改用户信息
function add_user($data,$time){
    ...
    if($userinfo){
		...
		if($ret){
			...
			$redis->set("nov:{$data['aid']}_rest_lottery_num:".$data['uid'],$rest,10*86400);			
		}
    }else {//新增
        $new_data = array(
			'uid' => $data['uid'],
		...
		if($ret){
			$redis->set("nov:{$data['aid']}_rest_lottery_num:".$data['uid'],intval($data['draw_data_num']),10*86400);	
		}
    }
    $redis->set("nov:{$data['aid']}_userinfo:".$data['uid'],$new_data,86400*10);
	return 	$ret;
}
