...
//task_work
$worker->addFunction("market_incremental_update", "market_incremental_update_func");
while($worker->work());
function market_incremental_update_func($job)
{
	...
	$option = array('where'=>$where,
	                  'table'=>'sj_market',
	                  'field'=>'apkurl,apksize,target_version_code,md5_file,id,version_code,real_version_code',
	);
	$new_market_info = $model->findOne($option, $server);
	...
	if(!empty($new_market_info))
	{
		...
		{   
		  	if(file_exists(APK_PREFIX.$new_market_info['apkurl']))
			{
				...
				$sql = "update sj_market set md5_file='{$new_market_info['md5_file']}' where id={$p['id']} limit 1";
				$model->query($sql);
			}
		}
        $sql = "select apkurl,apksize,target_version_code,id,md5_file,version_code,real_version_code  from sj_market where id in (".$old_version_id.")";
	    ...
	    while($old_market_info = $model->fetch($query))
			{
				...
				if(empty($old_market_info['md5_file'])){
					if (file_exists(APK_PREFIX. $old_market_info['apkurl'])) {
						...
						$sql = "update sj_market set md5_file='{$old_market_info['md5_file']}' where id={$old_market_info['id']} limit 1";
						$model->query($sql);
					}
				}
				$where = array('new_md5'=>$new_market_info['md5_file'],'old_md5'=>$old_market_info['md5_file'],"status"=>1);
				$option = array('where'	=>	$where,
	 					         		'table'	=>	'sj_market_patch',
	 					 				'field'	=>	'id');
				...
				$data = array(
				     '__user_table' => "sj_market_patch",
				     'new_market_id' => $new_market_info['id'],
				     'old_market_id' => $old_market_info['id'],
				     'new_version_code' => $new_market_info['real_version_code'],
				     'old_version_code' => $old_market_info['real_version_code'],   
				...
				*/
				// 客户端调整为发消息之前已经创建了记录，所以只更新不插入
				$update_where = array(
					'new_md5'=>$new_market_info['md5_file'],
					'old_md5'=>$old_market_info['md5_file']
				);
				$insert_flag = $model->update($update_where, $data);
				if(!$insert_flag)
				{   
