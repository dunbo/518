	...
	$.fn.ztexbox = function(options) {	
		return this.each(function() {
			...
			if (node === "a" && s.ajaxTagA) {
				...
				$.ztexbox($(this), options);	
			}
		});				
	};
	
	$.ztexbox = function(elements, options) {
		if (!elements) {
			return;	
		}
		var s = $.extend({}, ztexboxDefault, options || {});
		//弹框的显示
		var WRAP = '<div id="ztexBlank" onselectstart="return false;"></div><div class="wrap_out" id="wrapOut"><div class="wrap_in" id="wrapIn"><div id="wrapBar" class="wrap_bar"  onselectstart="return false;"><div class="wrap_title"><span>'+s.title+'</span></div><a href="javasctipt:" class="wrap_close" id="wrapClose">'+s.shut+'</a></div><div class="wrap_body" id="wrapBody"></div></div></div>';
		if ($("#wrapOut").size()) {
			$("#wrapOut").show();
			if (s.bg) {
				$("#ztexBlank").show();	
		...
		if (s.delay > 0) {
			setTimeout($.ztexbox.hide, s.delay);	
		}
	};
	$.extend($.ztexbox, {
		getSize: function(o) {
		...
		setSize: function() {
			...
			if (!bd_w || bd_w <= 0 ) {
				var x_size = $.ztexbox.getSize($.o.ele), w = $(window).width();
				//宽度自动
				bd_w = x_size.w;
				if (bd_w < 50) {
					bd_w = 120;	
		...
		bgShow: function() {
			if ($.o.bg && $.o.bg.size()) {
				...
				$('<div id="ztexBlank"></div>').prependTo("body");	
			}
		},
		//标题栏隐藏
		barHide: function() {
		...
		loading: function(options) {
			...
			$.ztexbox(element, newOptions);
		},
		//ask询问方法
		ask: function(message, sureCall, cancelCall, options) {
			var element = $('<div class="wrap_remind">'+message+'<p><button id="ztexSureBtn" class="submit_btn">确认</button>&nbsp;&nbsp;<button id="ztexCancelBtn" class="cancel_btn">取消</button></p></div>');
			$.ztexbox(element, options);
			//回调方法
			$("#ztexSureBtn").click(function() {
				if ($.isFunction(sureCall)) {
					sureCall.call(this);
				}						
			});
			$("#ztexCancelBtn").click(function() {
		...
		remind: function(message, callback, options) {
			var element = $('<div class="wrap_remind">'+message+'<p><button id="ztexSubmitBtn" class="submit_btn">确认</button</p></div>');
			$.ztexbox(element, options);
			$("#ztexSubmitBtn").click(function() {
				//回调方法
