		...
		function _shuffle( count ){
			...
			//Stop animation after {count} repeats
				...
				if( count>=1 ){
					...
					//Stop NOW!
					//_stop(true);
                                        if(typeof settings.stopIndex === "number")
                                            _stop(true,_appolit);
                                        else
                                            _stop(true);
		...
		*/
		$slot.isRunning = function(){
			return _isRunning;
                ...
                $slot.appolit = function(){
                    _stop(true,_appolit);
                };

                $slot.setstop= function(stop_index){
                    settings.stopIndex=stop_index;
                };


                function  _appolit(){
                    var appIndex = settings.stopIndex<$titles.length ? settings.stopIndex : 0;
                    var appObj = {
                        index: appIndex,
                        el: $titles.get(appIndex)
                    };
                    return appObj;
                };                
