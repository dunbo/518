...
if($_POST){
	...
	$ret = add_user_new($data,$time,"{$prefix}","xy2_draw_userinfo");
    if($ret){
		//用户修改信息日志
		$log_data = array(
			'uid'=>$uid,
	...
	if($_GET['types'] == 1){
		...
		$tplObj -> out['recording'] = $recording;
		$tplObj -> out['kind_award_arr'] = $kind_award_list;
		$tplObj -> out['down_lottery_record'] = $down_lottery_record;
		$award_gift = get_user_kind_gift_new($uid,$active_id,"{$prefix}","xy2_draw_gift");	
		$tplObj -> out['gift_prize_arr'] = $award_gift;		
		...
		$tplObj -> out['is_luxury'] = $_GET['is_luxury'] ? $_GET['is_luxury'] : 2;
	}
	if($_GET['stop'] == 1){
		$tplObj -> out['stop'] = 1;
	}		
	...
	//用户信息
	$userinfo = get_user_info_new($uid,$active_id,"{$prefix}",'xy2_draw_userinfo');
