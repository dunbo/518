<?php
class ExtentV2Action extends CommonAction {
	...
	function index()
	{
		...
		$sql = "SELECT * FROM  (SELECT  extent_id,extent_name,type,extent_size,oid,cid,show_form,parent_union_id,`status`,'belong_page_type_alias',rank,push_area,create_at,'is_resource_alias',extent_type FROM sj_extent_v2 where {$left_where} UNION ALL SELECT extent_id,extent_name,type,extent_size,oid,cid,-1,-1,`status`,belong_page_type,rank,'',create_at,is_resource,extent_type FROM sj_flexible_extent where {$right_where}) as A ORDER BY A.rank ASC,A.create_at DESC";
		$sql_count = "SELECT count(*) as count FROM  (SELECT  extent_id FROM sj_extent_v2 where {$left_where} UNION ALL SELECT extent_id FROM sj_flexible_extent where {$right_where}) as A ";
		$count_total = $model -> query($sql_count);
		...
		foreach ($list as $k => $v ) {
			if($v['show_form'] != -1 ) {
				if ($v['type'] == 1) {
					...
					$count = $model->table('sj_extent_soft_v2')->where($where)->count();
				...
				// 所属运营商
				$oid = $v['oid'];
				$list[$k]['oname'] = $oid ? $operators_key[$oid] : '不限';
				// 所属渠道名
				//V6.0渠道修改为多选
				$cid = $v['cid'];
					...
					if($list[$k]['chname']=="")
					{
						if($k1==0)
						{
							if(mb_strlen($v1['chname'],'utf-8')>10)
							{
								...
								$list[$k]['chname'].="<p>{$short_chname}</p>";
							}
							else
							{
	...
	function add_extent()
	{
		if (!empty($_POST)){
			...
			if($_POST['show_form'] == 5){
				$map['sform_data'] = $_POST['sform_data'] ? $_POST['sform_data'] : 1;
			}
			...
			isset($_POST['filter_rule']) && $map['filter_rule'] = $_POST['filter_rule'];
			...
			if ($id = $model->add($map)) {
				...
				//更新排序
				if ($map['type'] != 3)
					$this->_updateRankInfoExt($_POST['show_form'], $pid, $parent_id, $id, $rank);
					//$this->_updateRankInfo('sj_extent_v2', 'rank', $id, $where, $rank);
				...
				//$this->assign('jumpUrl', '/index.php/Sj/ExtentV2/index/pid/'. $pid);
				$msg='市场软件运营推荐-市场首页软件列表:添加了id为'.$id.'的区间';
				$msg .="可见渠道为[{$zh_chname}] \n";
				$msg .="运营商为[{$zh_oid_name}]";
				$this->writelog($msg, 'sj_extent_v2',"{$id}",__ACTION__ ,"","add");
				$this->success('添加成功');
			}
			...
			$count_1 = $extent_model -> where($map)->count();
    ...
    function add_multiSoftwareExtent() {
        if ($_POST) {
            ...
            if ($new_id) {
                $this->writelog("推荐位区间管理v1：添加id为【{$new_id}】多软件位区间",'sj_extent_v2',"{$new_id}",__ACTION__ ,"","add");
                $this->success('添加成功');
            } else {
                $this->error('添加失败');
            }
	...
	function edit_extent()
	{
		...
		if (!empty($_POST)){
			...
			$parent_id = !empty($_POST['parent_id'])?(int)$_POST['parent_id']:0;
			$where_rank['parent_id'] = $parent_id;
			if(!empty($_POST['cid'])){
				$new_where['status']=1;
			...
			//更新排序
			if ($old_extent['type'] != 3)
				//$this->_updateRankInfo('sj_extent_v2', 'rank', $id, $where_rank, $rank);
				$this->_updateRankInfoExt($old_extent['show_form'], $pid, $parent_id, $old_extent['extent_id'], $rank);
				//推送区域
			$pro_city=array();
        	$map['push_area'] = $_POST['area_value'];
			...
			if ($model->where($where)->save($map)) {
                ...
                //
				//	$model->table('sj_extent_soft_v2')->where($soft_where)->save($soft_data);
                //
				//	//活动区间会覆盖子区间的设置
				...
				$this->writelog($msg,'sj_extent_v2',"{$id}",__ACTION__ ,"","edit");
				//$this->assign('jumpUrl', '/index.php/Sj/ExtentV2/index/pid/'.$pid);
				$this->success('编辑成功');
			}
		} else {
			...
			$count_1 = $model -> where($condition)->count();
			$where_flexible = array(
					'belong_page_type'	=>	'fixed_homepage_recommend',
					'status'			=>	1,
					'pid'				=>	$pid,
    ...
    function edit_multiSoftwareExtent() {
        ...
        if ($_POST) {
            ...
            $log_msg = $this->logcheck($where, 'sj_extent_v2', $map, $model);
            $ret = $model->where($where)->save($map);
            if ($ret || $ret === 0) {
                if ($ret !== 0) {
                    $this->writelog('推荐位区间管理v1_编辑多软件位区间：' .$log_msg,'sj_extent_v2',"{$id}",__ACTION__ ,"","edit");
                }
                $this->success('编辑成功');
            } else {
                $this->error('编辑失败');
    ...
    function release_extent() {
        ...
        $this->writelog("推荐位区间管理v1：发布id为【{$extent_id}】的区间",'sj_extent_v2',"{$extent_id}",__ACTION__ ,"","edit");
        $this->success('发布成功');
    }
...
//更新某个排序
	function edit_rank(){
		if(isset($_GET)){
			//0：来至市场首页  1：来至灵活运营
			$extent_type = (int)$_GET['extent_type'];
			if($extent_type == 1) {
				//$table       = 'sj_flexible_extent';
				$field       = 'rank';
				...
				$param = $this->_updateRankInfoExt('-1', $pid, $parent_id, $extent_id, $target_rank, $lr, $p);
				$this -> writelog('在新版推荐位中更新了extent_id为'.$extent_id.'的区间', 'sj_flexible_extent', $extent_id,__ACTION__ ,"","edit");
				exit(json_encode(array('p'=>$p,'lr'=>$lr)));
			}else {
				//$table       = 'sj_extent_v2';
				$extent_id   = (int)$_GET['extent_id'];
				$parent_id   = $_GET['parent_id']?$_GET['parent_id']:0;
				...
				//更新排序
				//echo $extent_id;die;
				$extent_info = M('extent_v2')->where(array('extent_id'=>$extent_id))->find();
				$show_from = $extent_info['show_form'];
				$param = $this->_updateRankInfoExt($show_from, $pid, $parent_id, $extent_id, $target_rank, $lr, $p);
				$this -> writelog('在新版推荐位中更新了extent_id为'.$extent_id.'的区间', 'sj_extent_v2', $extent_id,__ACTION__ ,"","edit");
...
function add_soft()
	{
		...
		$model = M('extent_soft_v2');

		$extent = $model->table('sj_extent_v2')->where(array('extent_id' => $_REQUEST['extent_id']))->find();
		$address_edit_able = true;
		if (!empty($_POST)){
            ...
            if (!$qualified_flag) {
                $msg = $error_msg[0]['msg'];
                // 业务逻辑：设置返回的跳转页面
                $this->assign('jumpUrl', '/index.php/Sj/ExtentV2/list_soft/extent_id/'. $_POST['extent_id'].'/show_form/'.$show_form);
                $this->error($msg);
            }
			...
			$resource_type = $_POST['resource_type']?$_POST['resource_type']:0;
			$is_dev = $_POST['is_dev']?$_POST['is_dev']:0;
			$is_tag = $_POST['is_tag']?$_POST['is_tag']:0;

			//推荐2 
			if($show_form == 1 || $show_form == 4) {
				if( $is_tag == 1 ) {
			...
			if($show_form == 1 || $show_form == 4) {
				...
				if( $is_tag == 1 ) {
					...
					if($rcontent_2==true)
					{
						...
						//6.4.5
						$conetnt_id_2 = M('')->table('sj_flexible_extent_soft')->add($conetnt_map_2);
						$map['c_id_2'] = $conetnt_id_2;
						$map['is_tag'] = $is_tag;
			...
			//同步数据到灵活运营表
			if(!$resource_id && in_array($show_form, array(1,3,4))) {
				$_POST['new_package']	=	$_POST['package_ext'];
				$_POST['package_name']	=	$_POST['soft_name'];
				$_POST['title'] 		=	$_POST['content_title'];
				$_POST['extent_v2_id']	=	$_POST['extent_id'];
				$_POST['resource_id']	=	null;
				$_POST['extent_id']		=	null;
				$_POST['data'] = $map;
				if($show_form == 1) {
					$_POST['extent_type']= 24;
				}elseif($show_form == 3) {
					$_POST['extent_type']= 26;
				}elseif($show_form == 4) {
					$_POST['extent_type']= 29;
				}
				$action_name = C('FROM_ACTION');
				A($action_name['name'],'@','add_soft','ExntentV2');
				exit();
			}
			
			if($resource_id) {
				$map['c_id_1']	=	$resource_id;
			}
			
			if ($id = $model->add($map)) {
				$this->assign('jumpUrl', '/index.php/Sj/ExtentV2/list_soft/extent_id/'.$extent_id.'/show_form/'.$show_form);
				$this->writelog("市场软件运营推荐-市场首页软件列表:在区间ID为[{$extent_id}]中添加了软件[{$_POST['package_ext']}],显示概率为{$_POST['prob']},开始时间为{$_POST['start_at']},结束时间为{$_POST['end_at']},合作方式为{$_POST['type']},", 'sj_extent_soft_v2',$id,__ACTION__ ,"","add");
				...
				//$this->assign('jumpUrl', '/index.php/Sj/ExtentV2/index');
				$this->error('添加失败');
			}
		} else {
			$util = D("Sj.Util");
            ...
            $find = $model->table('sj_extent_v2')->where($where)->find();
            $extent_type = 1;
            if ($find) {
	...
	function list_area_soft()
	{
		...
		$extents = $model->table('sj_extent_v2')->where($extent_where)->select();
		$extent_ids = array();
		...
		$extent_result = $extents = $model->table('sj_extent_v2')->where($where)->order('parent_id asc, rank asc, type desc')->select();
		$extent_list = array();
	...
	function list_soft()
	{
		...
		$extent = $model->table('sj_extent_v2')->where(array('extent_id' => $extent_id))->find();
		$pid = $extent['pid'];
		$type = $extent['type'];
        $extent_type = $extent['extent_type'];
		$this->assign('pid', $pid);
	...
	function edit_soft()
	{
		...
		$model = M('extent_soft_v2');
		if(in_array($show_form, array(1,3,4))) {
			$sql  = "SELECT A.id,A.extent_id,A.package,A.prob,A.start_at,A.end_at,A.type,A.phone_dis,A.old_phone,A.default_display,A.beid,A.is_brush,A.anzhi_ad_type,A.is_tag,A.tag_title,A.c_id_1,A.c_id_2,";
			$sql .= "B.id as f_id,B.extent_id as from_resource,B.title as content_title,B.content_type,B.package_643,B.is_dev,B.resource_type,B.video_url,B.video_pic ";
			$sql .= "FROM sj_extent_soft_v2 as A LEFT JOIN sj_flexible_extent_soft as B on A.c_id_1=B.id WHERE A.id = {$id} and A.`status`=1";
			$soft = M('')->query($sql);
			$soft = $soft[0];
		}else {
		...
		//echo $sql;die;
		$extent = $model->table('sj_extent_v2')->where(array('extent_id' => $soft['extent_id']))->find();
        $extent_type = $extent['extent_type'];
        $this->assign('extent_type', $extent_type);
		$address_edit_able = false;
		if (!empty($_POST)){
			...
			$log_msg = $this->logcheck($where, 'sj_extent_soft_v2', $map, $model);
			if($map['package']){
				//屏蔽软件上排期时报警需求 新增  yuesai
		        $AdSearch = D("Sj.AdSearch");
		        $shield_error=$AdSearch->check_shield($map['package'],$map['start_at'],$map['end_at']);
			...
			if($_POST['life']==1)
			{
				$select=$model->where($where)->find();
				$map['status']			=	1;
				$map['create_at']		=	time();
				$map['extent_id']		=	$select['extent_id'];
				$map['c_id_1']			=	$select['c_id_1'];
				$map['c_id_2']			=	$select['c_id_2'];
				$map['resource_type']	=	$select['resource_type'];
				$ret=$model->table('sj_extent_soft_v2')->add($map);
			   if ($ret) 
				{
					$this->assign('jumpUrl', '/index.php/Sj/ExtentV2/list_soft/extent_id/'. $soft['extent_id'].'/show_form/'.$show_form);
					//$this->writelog("推荐位区间管理v2：{$log_msg}");
					$this->writelog("在新版推荐位中复制上线了软件[".$_POST['package_ext']."]".$log_msg, "",$_POST['package_ext'], 'sj_extent_soft_v2',$ret,__ACTION__ ,"","add");
					// 检查导入的区间中有没有软件数大于区间位置数的，有的话要发邮件提醒运营
					$check_extent = array(
						$_POST['extent_id'] => array(
							'start_at' => $map['start_at'],
				...
				$is_dev = $_POST['is_dev']?$_POST['is_dev']:0;
				$is_tag = $_POST['is_tag']?$_POST['is_tag']:0;
				$tag_title = $_POST['tag_title']?trim($_POST['tag_title']):0;
				...
				if($show_form == 1 || $show_form == 4) {
					...
					if( $is_tag == 1 ) {
						...
						{
							$conetnt_map_2['update_at'] = time();
							if(!empty($soft['c_id_2'])) {
								$where_flex = array('id'=>$soft['c_id_2']);
								$res_flex = M('')->table('sj_flexible_extent_soft')->where($where_flex)->save($conetnt_map_2);
								if($res_flex) {
									$conetnt_id_2 = $soft['c_id_2'];
								}else {
									$this->error('编辑失败');
								}
							}else {
				...
				if(!$resource_id && in_array($show_form, array(1,3,4))) {
					...
					$action_name = C('FROM_ACTION');
					A($action_name['name'],'@','edit_soft','ExntentV2');
					exit();
				}
				...
				{
					$this->assign('jumpUrl', '/index.php/Sj/ExtentV2/list_soft/extent_id/'. $soft['extent_id'].'/show_form/'.$show_form);
					$this->writelog("市场软件运营推荐- 市场首页软件列表:编辑了软件[".$_POST['package_ext']."]".$log_msg,'sj_extent_soft_v2',$id,__ACTION__ ,"","edit");
					// 检查导入的区间中有没有软件数大于区间位置数的，有的话要发邮件提醒运营
					$check_extent = array(
						$_POST['extent_id'] => array(
							'start_at' => $map['start_at'],
			...
			$content_list_one = M('')->table('sj_flexible_extent_soft')->where(array('id' => $soft['c_id_1']))->find();
			$content_list_two = M('')->table('sj_flexible_extent_soft')->where(array('id' => $soft['c_id_2']))->find();
			$util = D("Sj.Util");
	...
	function del_extent()
	{
		...
		//重新排序
		$this->_updateRankInfoExtDel($extent['pid'], $parent_id);
		
		$this->writelog('市场软件运营推荐-市场首页软件列表:删除了id为'.$extent_id.'的区间','sj_extent_v2',$extent_id,__ACTION__ ,"","del");
		$this->success('删除成功');
	}
	...
	function cancel_union()
	{
		...
		if ($flag)
		{
			$this->writelog('在新版推荐位中修改了id为'.$extent_id.'的区间','sj_extent_v2',$extent_id,__ACTION__ ,"","edit");
			$this->success('取消联合成功');
		}
		else
		{
	...
	function move_soft()
	{
		...
		$this->writelog("在新版推荐位中将id为[{$selected_ids}]的软件移动到了区间{$extent_id}", 'sj_extent_soft_v2',$selected_ids,__ACTION__ ,"","edit");
        // 检查导入的区间中有没有软件数大于区间位置数的，有的话要发邮件提醒运营
        ...
        foreach ($selected_ids_arr as $id) {
            ...
            $find = $model->table('sj_extent_soft_v2')->where($where)->find();
            if (!$find)continue;
	...
	function del_soft()
	{
		...
		$model->where($where)->save($map);
		$this->writelog("市场软件运营推荐-市场首页软件列表:删除了id为[$id]包名为{$package['package']}的区间推荐软件", 'sj_extent_soft_v2', $id,__ACTION__ ,"","del");
		$this->success('删除成功');
	}
	...
	function checkPropAct()
	{
		...
		$result = $model->table('sj_extent_v2')->where($where)->find();
		...
		if (!empty($_POST)){
			...
			if(empty($resource_id)){
				...
				if($rcontent_1==true)
				{
					...
					//6.4.5
					$conetnt_id_1 = M('')->table('sj_flexible_extent_soft')->add($conetnt_map_1);
					$map['c_id_1'] = $conetnt_id_1;
				}else {
			...
			if ($id = $model->add($map)) {
				// $this->assign('jumpUrl', '/index.php/Sj/ExtentV2/list_candidate_soft');
				$this->writelog("在新版推荐位中添加了id为[{$id}]包名为{$_POST['package_ext']}的备选软件", 'sj_extent_candidate_v2', $id,__ACTION__ ,"","add");
				$this->success('添加成功');
			} else {
				$this->assign('jumpUrl', '/index.php/Sj/ExtentV2/list_candidate_soft');
				$this->error('添加失败');
	...
	function edit_candidate_soft()
	{
		...
		if (!empty($_POST)){
			...
			if ($model->where($where)->save($map)) {
				// $this->assign('jumpUrl', '/index.php/Sj/ExtentV2/list_candidate_soft');
				$this->writelog("在新版推荐位中编辑了id为[{$id}]包名为{$_POST['package_ext']}的备选软件", 'sj_extent_candidate_v2', $id,__ACTION__ ,"","edit");
				$this->success('编辑成功');
			}
		} else {
			if( $soft['c_id_1'] ) {
	...
	function del_candidate_soft()
	{
		...
		if($_POST['ids']){
			...
			$this->writelog("在新版推荐位中删除了id为[{$ids}]的备选软件", 'sj_extent_candidate_v2', $ids,__ACTION__ ,"","del");
			$this->success('删除成功');
		}
		$id = $_REQUEST['id'];
		$where = array(
		...
		$package = $model->where($where)->find();
		$model->where($where)->save($map);
		$this->writelog("在新版推荐位中删除了id为[{$id}]包名为{$package['package']}的备选软件", 'sj_extent_candidate_v2', $id,__ACTION__ ,"","del");
		$this->success('删除成功');
	}
...
//			var_dump($ids);
			...
			$old_info = $model->table('sj_extent_candidate_v2')->where($where)->field('id,package,start_at,end_at')->select();
...
//			var_dump($edit_id);
			$pack_info =  $model->table('sj_extent_candidate_v2')->where(array('package'=>array('in',$package),'id'=>array('not in',$ids),'status'=>1))->field('id,package,start_at,end_at')->select();
...
//				var_dump($val['start_at'].'--'.$val['end_at'].'--'.$edit_id[$val['package']]['start_at'].'--'.$edit_id[$val['package']]['end_at']);
			...
			$res = $model->table('sj_extent_candidate_v2')->where(array('id'=>array('in',$can_id)))->save($data);

			if($res){
				$log_id = implode(',',$can_id);

				$this->writelog("批量修改了备选软件列表的时间id为{$log_id}的{$log_tm}", 'sj_extent_candidate_v2',$log_id,__ACTION__ ,"","edit");
    ...
    function pub_update_ad_info(){
        ...
        foreach($info as $k=>$v){
            ...
            // $pkg_data=$model->table('sj_extent_candidate_v2')->where(array('package'=>$k,'status'=>1))->find();
            // if($pkg_data){
            // 	$repeat_pack[]=$k;
            // 	continue;
            // }
            ...
            $data_error = $model->table("sj_extent_candidate_v2")->where(array('package'=>$k,'start_at'=>array('lt',$data['end_at']),'end_at'=>array('gt',$data['start_at']),'status'=>1))->find();
			if($data_error){
				$repeat_pack[]=$k;
            	continue;
			}
			/*$if_new = false;
			$if_new_arr = $model->table("sj_extent_candidate_v2")->where(array('package'=>$k,'status'=>1))->select();
			...
			if($if_new == false){*/
				$sql = "SELECT b.* FROM sj_flexible_extent as a INNER JOIN sj_flexible_extent_soft as b on a.extent_id = b.extent_id WHERE a.belong_page_type= 'fixed_resource_channel' and a.status=1 and b.resource_type= 24 and b.status = 1 and b.package_643 ='{$k}'";
				$common_jump = $model->query($sql);
				//$common_jump = $model->table('sj_common_jump')->field('id,content_type')->where(array('status'=>1,'package_643'=>$k,'resource_type'=>24))->select();
				if(!empty($common_jump)){
					...
					$data['c_id_1'] = $resource_id;
					$data['resource_type'] = 24;
				}
			/*}*/
            $res = $model->table('sj_extent_candidate_v2')->add($data);
            if($res){
                $import_count++;
                $id_str .= $res.',';
            }
        ...
        if($id_str){
            $this->writelog($i_log."id为{$id_str}的数据", 'sj_extent_candidate_v2', $id_str,'/index.php/Dev/SoftWhitelist/edit_more_ad','','add');
        }
        $count=count($pkg);
	...
	function pub_check_extent_address()
	{
		...
		//	$sql = "select count(*) as total from sj_extent_soft_v2 where extent_id='{$extent_id}' and location<>'' ";
		//	$res = $model->query($sql);
		//	if ($res[0]['total'] > 0) {
		//		exit('0');
		//	}
		//	if ($extent['type'] == 2) {
		//		$sql = "select count(*) as total from sj_extent_v2 where parent_id='{$extent_id}' and location<>'' ";
		//		$res = $model->query($sql);
		//		if ($res[0]['total'] > 0) {
		//			exit('0');
		//		}
	...
	function phone_config_manage(){
		...
		if($result){
			$this->assign('jumpUrl', '/index.php/Sj/ExtentV2/phone_config_manage_show');
    ...
    function handwriting_convert_and_check(&$content_arr) {
        ...
        /*foreach ($content_arr as $key => $record) {
        	...
        	if( !isset($record['content_title']) && empty($record['content_title']) ) {
        		...
        		$find = M('extent_v2')->where($where_extent)->find();
        		if( $find['show_form'] != 1 ) {
    ...
    function logic_check($content_arr) {
        ...
        // 业务逻辑：区间表、区间软件表
        $M_extent_table = 'extent_v2';
        $M_extent_soft_table = 'extent_soft_v2';
        ...
        foreach($content_arr as $key=>$record) {
            ...
            if (!isset($record['bk_cid']) || !isset($record['bk_oid']) || !isset($record['bk_pid']))
                continue;
            // 检查是否与sj_extent_soft_v2表里有相同包名且区间冲突的包
            ...
            if (isset($record['id'])&&$record['life']!=1) {
                ...
                // 拼接sql，A为下面的$sql里表sj_extent_soft_v2的别名，注意二者需一致
    ...
    function import_array_convert_and_check(&$content_arr) {
        ...
        //屏蔽软件上排期时报警需求 新增  yuesai
		$AdSearch = D("Sj.AdSearch");
        $error_msg3 = $AdSearch->logic_check_shield($content_arr,'start_at','end_at','sj_extent_soft_v2');
    ...
    function import_softs() {
        ...
        if ($_GET['down_moban']) {
            ...
            // 判断后台有没有人正在导入
            $lock_name = 'sj_extent_soft_v2_importing';
    ...
    function process_import_array($content_arr) {
        ...
        foreach($content_arr as $key => $record) {
            ...
            if($record['content_type'] != 0){
            	...
            	$sql = "SELECT b.* FROM sj_flexible_extent as a INNER JOIN sj_flexible_extent_soft as b on a.extent_id = b.extent_id WHERE a.belong_page_type= 'fixed_resource_channel' and a.status=1 and b.resource_type= 24 and b.package_643 ='{$where['package_643']}' and b.content_type ={$where['content_type']} and b.status = 1";
            	$common_jump_one = M('')->query($sql);
            	//$common_jump_one = M('')->table('sj_flexible_extent_soft')->field('id')->where($where)->find();
            	if(!empty($common_jump_one)){
            		$resource_id = $common_jump_one[0]['id'];
            	}
            }
            if($record['content_type']==0){
            	...
            	$sql_2 = "SELECT b.id,b.content_type  FROM sj_flexible_extent as a INNER JOIN sj_flexible_extent_soft as b on a.extent_id = b.extent_id WHERE a.belong_page_type= 'fixed_resource_channel' and a.status=1 and b.resource_type= 24 and b.status = 1 and b.package_643 ='{$where['package_643']}' ";
            	$common_jump = M('')->query($sql_2);
        		//$common_jump = $model->table('sj_flexible_extent_soft')->field('id,content_type')->where($where)->select();
				if(!empty($common_jump)){
					$tmp_arr = array();
					foreach($common_jump as $val){
						$tmp_arr[$val['content_type']] = $val['id'];
            ...
            // 添加到表中
			if ($id = $model->add($map)) {
				$this->writelog("在新版推荐位中区间[{$record['extent_id']}]中添加了软件[{$record['package']}],显示概率为{$record['prob']},开始时间为{$record['start_at']},结束时间为{$record['end_at']},合作方式为{$record['type']}", 'sj_extent_soft_v2', $id, __ACTION__, "", "add");
        ...
        if(count($arr_shields) && $file_data=$AdSearch->generate_ignore_file($arr_shields,'sj_extent_soft_v2')){
        	$result_arr['table_name'] = 'sj_extent_soft_v2';
	...
	public function pub_shield_new_ignore(){
		...
		$model = M('');
		 $table_config=array(
            ...
            "sj_extent_soft_v2"=>array("sj_extent_soft_v2",'start_at','end_at',"市场首页软件列表",'package'),
