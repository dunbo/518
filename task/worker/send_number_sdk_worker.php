...
$worker->addFunction("send_number_sdk", "send_number_func");
...
function send_number_func($jobs) {
	...
	$num_key = "sdk_active{$active_id}_uid{$uid}_num";//结果缓存key
	//$prize_key = "prize_list_active{$active_id}";//活动的奖品池
	$prize_key = "NUMBER_LIST:{$active_id}";//活动的奖品池
	$order_key = "sdk_order_list_active{$active_id}_date{$date}";//预约列表
	...
	if ($sended_num >= $active_info['sdk_conf_cnt']) {
...
function get_active_usedcnt($active_id, $type, $date, $ip, $uid) {
	global $db;
	$option = array(
