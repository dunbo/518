...
$login_url = $center_url."http://promotion.anzhi.com/lottery/bomb_island_reserve/index.php?lgfrom=1&".$build_query;
...
if((isset($_SESSION['USER_UID']) && $_SESSION['USER_ID'] != 13176))//已登录
{
	...
	//todo 是否预约
	$res = $redis->get("bomb_island_reserve:{$active_id}_is_sign:".$uid);
	if($res == 1)
	{
	...
	else
	{
		if($_GET['lgfrom']) //不管是登录还是注册都是预约成功的
		{
			...
			$redis->set("bomb_island_reserve:{$active_id}_is_sign:".$uid,1,86400*30);
...
$tpl = "lottery/bomb_island_reserve/index.html";
if($_SERVER['SERVER_ADDR'] == '118.26.203.23')
{
...
$tplObj -> out['share_text'] = "休闲竞技手游《弹弹岛2》预约领豪华大礼！";
