...
include_once GO_APP_ROOT.DS.'..'.DS.'cron2'.DS.'cache'.DS.'function.php';
//define('STATDB', 'slave');
...
$product_key = $_POST['pro_key'] ? $_POST['pro_key'] : 'top_config';
$option = array('table' => 'pu_config','where' => array('config_type' => $product_key,'configname'=>'top_config'));
...
if(!$config_info){
	...
	$data['configname'] = 'top_config';
	$data['config_type'] = $product_key;
	$data['configcontent'] = json_encode($info);
	$data['uptime'] = time();
	$affect = $model -> insert($data);
		writelog('添加 configname为 top_config 的配置');
}else{
	$info = json_decode($config_info['configcontent'],true);
		$where['configname'] = 'top_config';
	$where['config_type'] = $product_key;
	if($_POST['gongshi']){
	...
	writelog('修改top_config 的参数配置 修改参数为 '.$info_json,'pu_config',"config_type:top_config",__ACTION__ ,"","edit");
}
}
...
if($_POST['act'] == 'daily'){
	...
	$data = array();
	$current_time = $_POST['currdate'] ? strtotime($_POST['currdate']) : strtotime(date('Y-m-d',time()));
...
if(in_array($_POST['act'],array('daily','weekey'))){
	...
	foreach($where_time_arr as $where_time){
		$date = date('Y-m-d',$where_time);
		$result = get_package_by_date_download($date);
		foreach($result as $pkg => $dl_cnt){
			$softdl[$pkg][$date] = $dl_cnt;
			$softdl[$pkg]['sum'] += $dl_cnt;
			$softdl_sum[$pkg] += $dl_cnt;
		}//softdl以上是下载量源
