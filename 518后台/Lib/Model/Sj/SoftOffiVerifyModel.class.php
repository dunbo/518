...
class SoftOffiVerifyModel extends Model {
	...
	public function _search() {
		...
		$this->condition .= ' AND SR.official_bitwise = SR.official_bitwise | ' . $this->offi_bit;
		...
		if (($mainType = $_GET['searchMainType']) && ($mainStr = $_GET['searchMainStr'])) {
			switch ($mainType) {
				case '1':
					...
					break;
				case '2':
					$this->condition .= ' AND (SS.package like "' . "%{$mainStr}%" . '")';
					break;
				case '3':
					$this->condition .= ' AND (SS.softid like "' . "%{$mainStr}%" . '")';
					break;
				case '4':
					$this->condition .= ' AND (SS.dev_name like "' . "%{$mainStr}%" . '")';
					break;
				case '5':
					$this->condition .= ' AND (SS.dever_email like ' . "%{$mainStr}%" . '")';
					break;
			}	
		}
		...
		//日期
		if ($s_tm = $_GET['s_tm']) {
			if ($_GET['datelineType'] == 1) {
				$this->condition .= ' AND (SR.dateline >=' . strtotime($s_tm . ' 00:00:00') . ')';
			}
		}
		if ($e_tm = $_GET['e_tm']) {
			if ($_GET['datelineType'] == 1) {
				$this->condition .= ' AND (SR.dateline <=' . strtotime($e_tm . ' 23:59:59') . ')';
			}
		}
	...
	public function getSoftCategoryOne($cid) {
		$result = $this->query("select CONCAT(PCG.name, '|', CCG.name) AS name from sj_category as CCG 
				left join sj_category as PCG on CCG.parentid = PCG.category_id where CCG.category_id = {$cid}");	
		return $result[0];
	}
	/**
	 * 得到开发者信息
	...
	public function saveSign($data) {
		$this->query("insert sj_soft_sign(package, sha1_file, md5_file, sign, dateline, status)
				values('{$data['package']}', '{$data['sha1_file']}', '{$data['md5_file']}',
				'{$data['sign']}', '{$data['dateline']}', '{$data['status']}')");
	}
