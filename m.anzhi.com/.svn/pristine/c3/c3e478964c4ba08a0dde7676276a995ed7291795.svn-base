...
$url = "http://promotion.anzhi.com/lottery/ghost_sign_lottery/index.php?".$build_query;
...
if($_POST){
	...
	if($_GET['types'] == 1){
		//我的奖品
		$kind_award_list = get_user_kind_award_new($uid,$active_id,'ghost_sign_lottery','valentine_draw_award');
		foreach($kind_award_list as $k => $v)
		{
			if($v['prizename']=='5元礼券')
			{
				$get_5_gift_arr[$k]['uid'] = $v['uid'];
				$get_5_gift_arr[$k]['pid'] = $v['pid'];
				$get_5_gift_arr[$k]['prizename'] = $v['prizename'];
				$get_5_gift_arr[$k]['time'] = $v['time'];
				unset($kind_award_list[$k]);
			}
		}
		$tplObj -> out['kind_award_arr'] = $kind_award_list;
		$tplObj -> out['get_5_gift_arr'] = $get_5_gift_arr;
		/*$kind_award_gift = get_user_kind_gift_new($uid,$active_id,"ghost_sign_lottery","valentine_draw_gift");
	...
	//用户信息
	$userinfo = get_user_info_new($uid,$active_id,"ghost_sign_lottery","valentine_draw_userinfo");
	...
	$tplObj -> display('lottery/ghost_sign_lottery/userinfo.html');	
}
