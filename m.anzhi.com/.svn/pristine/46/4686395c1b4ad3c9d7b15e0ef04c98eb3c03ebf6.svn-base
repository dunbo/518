...
/*$activity_option = array(
    'where' => array(
        'config_type' => 'Rio_Olympic_LOTTERY_AWARD',
        'status' => 1
    ),
    'cache_time' => 600,
    'table' => 'pu_config'
...
$fix_package = '["com.nuomi","com.achievo.vipshop","com.rd.zdbao.adw","com.tuan800.tao800"]';
...
$rkey_imsi_puzzle = "rio_olympic:{$aid}:{$imsi}:".$date;//用户每天拼图次数
$rkey_imsi_lottery_num = "rio_olympic:{$aid}:{$imsi}:rest_lottery_num";//用户可抽奖次数
$rkey_imsi_package_list = "rio_olympic:{$aid}:{$imsi}:package_list"; //下载的包
$rkey_imsi_pics_list = "rio_olympic:{$aid}:{$imsi}:pics_list"; //图片
	...
	if (empty($lottery_num) && $imsi) {
		...
		if ($find) {
			...
			$imsi_data = array(
				...
				'lottery_num'=>0,
				'time' => time(),
				'__user_table' => 'rio_olympic_lottery_num',
			);
...
function get_pics($aid){
	...
	//if($puzzle_pics === null){	
		...
		$activity_option = array(
			...
			//'field' => 'img1,img2,img3',
		);
		$puzzle_result = $model -> findOne($activity_option);
		if($puzzle_result['img1'])
		{
			$puzzle_pics[] = $puzzle_result['img1'];
		}
		if($puzzle_result['img2'])
		{
			$puzzle_pics[] = $puzzle_result['img2'];
		}
		if($puzzle_result['img3'])
		{
			$puzzle_pics[] = $puzzle_result['img3'];
		}
		
		$redis->set($rkey_imsi_pics_list,$puzzle_pics,10*86400);
	//}
	return $puzzle_pics;			
}
//根据获奖等级得到奖品名称
