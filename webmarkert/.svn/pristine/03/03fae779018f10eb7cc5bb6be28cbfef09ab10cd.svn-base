...
//软件:1:最佳应用(14),2:最受欢迎(14),3:最佳游戏(4),4:最受欢迎游戏(4),5:优秀应用(10),6:优秀游戏(10)
$num_limit = array(
	1 => 14,
	2 => 14,
	3 => 4,
...
$year_query = mysql_query("SELECT * FROM sl_year_list WHERE year = 2012  AND status = 1 ORDER BY year ");
...
//现场图片
$query = mysql_query("SELECT * FROM sj_year_scenephoto WHERE year = 1 ORDER BY id ASC LIMIT 0,16",$link);
while($rs = mysql_fetch_array($query,MYSQL_ASSOC)) {
...
//现场图片_查看更多
$query = mysql_query("SELECT * FROM sj_year_scenephoto_more WHERE year = 1",$link);
$rs = mysql_fetch_array($query,MYSQL_ASSOC);
$scenephoto_more = $rs['url'];

//合作媒体
$query = mysql_query("SELECT * FROM sj_year_mediapartners WHERE year = 1 ORDER BY id ASC");
while($rs = mysql_fetch_array($query,MYSQL_ASSOC)) {
...
require_once(ROOT.'tpl/index.htm');
