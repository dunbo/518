...
if(!isset($active_id)){
	...
	$url = $center_url."http://promotion.anzhi.com/lottery/ghost/index.php?aid=".$active_id."&sid=".$sid;
	header("Location: {$url}");
}
...
if($_GET['stop'] != 1){
	...
	if(!$res){
		$url = $center_url."http://promotion.anzhi.com/lottery/ghost/index.php?stop=1&aid=".$active_id."&sid=".$sid;
		header("Location: {$url}");
	}
}
//获取活动配置
function get_config($aid,$uid){
	global $model;
	global $redis;	
	$tm_config = $redis->get("ghost:{$aid}_tm_config:".$uid);	
	if($tm_config === null){	
		$option = array(
			'where' => array(
				'id' => $aid,
		...
		$redis->set("ghost:{$aid}_tm_config:".$uid,$tm_config,10*86400);
	}
	return $tm_config;			
}
//获取抽奖次数
function get_lottery_num($aid,$uid){
	global $model;
	global $redis;		
	$lottery_num = $redis->get("ghost:{$aid}_rest_lottery_num:".$uid);
	if($lottery_num === null){
		$option = array(
			'where' => array(
				'uid' => $uid,
		...
		$redis->set("ghost:{$aid}_rest_lottery_num:".$uid,intval($lottery_num),30*86400);		
	}	
	return $lottery_num;		
}
//添加用户、修改用户信息
function add_user($data,$time){
    ...
    if($userinfo){
		...
		if($ret){
			...
			$redis->set("ghost:{$data['aid']}_rest_lottery_num:".$data['uid'],$rest,30*86400);			
		}
    }else {//新增
        $new_data = array(
			'uid' => $data['uid'],
		...
		if($ret){
			$redis->set("ghost:{$data['aid']}_rest_lottery_num:".$data['uid'],intval($data['draw_data_num']),30*86400);	
		}
    }
    $redis->set("ghost:{$data['aid']}_userinfo:".$data['uid'],$new_data,86400*30);
	return 	$ret;
}
