...
function add_async_download_listen_script(port, js_param, php_param) {
	...
	if (typeof(js_param['callback'])=='undefined') {
		js_param['callback'] = 'change_install_status';
	}
    var url = 'http://127.0.0.1:' + port + '/query?r=' + Math.floor(Math.random() * ( 1000000000 + 1));
    // 根据js_obj继续完善url
    ...
    m.onload = function() {
        if (!market_installed) {
            listen_failed_count++;
            if (listen_failed_count >= listen_failed_max && typeof(php_param['disable_auto']) == 'undefined') {
