...
$worker->addFunction("custom_gift_work", "get_award");
while ($worker->work());
get_award();
function get_award($jobs){
	...
	$prefix = "custom";		
	$model -> query("DELETE FROM {$table} WHERE aid={$aid} and pid={$pid}",'lottery/lottery');
	//激活码入口
	foreach ($data_arr as $key => $val) {
		if ($val) {
			...
			$sql = "INSERT INTO {$table} (`aid`,`pid`,`gift_number`,`create_tm`,`update_tm`,`package`,`softname`) VALUES ".$sql_str;
			$affect = $model -> query($sql,'lottery/lottery');
		}
	}
	get_gift($prefix,$aid,$pid,$table);
}
//刷礼包缓存
function get_gift($prefix,$aid,$pid,$table){
	global $model;
	global $redis;	
    $redis->pingConn();			
	...
	shell_exec("/usr/local/bin/redis-cli -h {$ip} -p {$port}  keys  '{$prefix}:{$aid}:{$pid}_*' | xargs /usr/local/bin/redis-cli -h {$ip}  -p {$port} DEL");	
	...
	if(!$prize_gift_pkg){
		...
		for($start=0;;$start++){
			...
			foreach($prize_gift as $k => $v){
				$gift_key = "{$prefix}:{$aid}:{$pid}_gift:".$k;
