...
if(isset($_SESSION['USER_UID'])){//已登录
	...
	$url = "http://promotion.anzhi.com/lottery/nov_sign/index.php";
...
if($_POST){
	//剩余抽奖次数
	$now_num = $redis->setx('incr',"nov:{$active_id}_rest_lottery_num:".$uid,-1);
	if($now_num < 0){
		$redis->set("nov:{$active_id}_rest_lottery_num:".$uid,0,30*60);
	...
	$new_array['package'] = get_gift_pkg($active_id,$uid,$_POST['pkg'],"nov");
	$the_award = $task_client->do('nov_sign', json_encode($new_array));	
	$lottery_rs = json_decode($the_award,true);		
	if($lottery_rs['pid'] == 0){
		$gift_info = json_decode($lottery_rs['gift_number'],true);
		del_gift_pkg($active_id,$uid,$gift_info['package'],"nov");
	}	
	//用户已用抽奖次数+1
	save_deduction_num_new($uid,$active_id,$_SESSION['USER_NAME'],"nov","valentine_draw_userinfo");
	if($the_award == -1){
	...
	if($lottery_rs['pid'] == 0){
		...
		//礼包的所有兑换信息
		$key = "nov:{$active_id}_gift_prize:{$uid}";
		...
		$key = 	"nov:{$active_id}_draw_award:{$uid}";
		$redis -> lPush($key,json_encode($arr));
		$redis -> expire($key,86400*10);
		$arr['username'] = str_replace_cn_new($_SESSION['USER_NAME'], 1, -2 );
		$award_key = "nov:{$active_id}_draw_award";
